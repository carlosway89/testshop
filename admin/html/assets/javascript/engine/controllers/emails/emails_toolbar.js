/* --------------------------------------------------------------
 emails_toolbar.js 2015-10-15 gm
 Gambio GmbH
 http://www.gambio.de
...
gx.controllers.module(
	...
	function (data) {
		...
		var
			...
			module = {
				model: {
					settings: jse.core.config.get('shopUrl') + '/admin/admin.php?do=Emails/GetEmailSettings'
				}
			};
		...
		var _onCreateNewEmail = function (event) {
			...
			if (typeof module.model.settings !== 'undefined' && module.model.settings !== null) {
				...
				if (module.model.settings.signature !== null && module.model.settings.signature !== '') {
					var signatureHtml = '<br><p>-----<br>' + module.model.settings.signature + '</p>';
		...
		module.init = function (done) {
			...
			// Check if the "mail_to" parameter is present and process its value within the
			// new email modal layer.
			var getParameters = jse.libs.url_arguments.getParameterArray();
