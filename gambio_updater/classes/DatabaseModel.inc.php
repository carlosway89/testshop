<?php
/* --------------------------------------------------------------
   DatabaseModel.inc.php 2015-06-25 gm
   Gambio GmbH
   http://www.gambio.de
...
class DatabaseModel
{
	...
	public function set_index($p_table, $p_index_type, $p_columns_array, $p_index_name = null)
	{
		...
		$t_get_indexes_query = $this->query('SHOW INDEX FROM `' . $p_table . '` WHERE `Non_unique` = ' . $t_non_unique);

		foreach($t_get_indexes_query as $value)
		{
			$t_column_array = array();
			$t_get_key_name_columns = $this->query('SHOW INDEX FROM `' . $p_table . '` WHERE `Key_name` = "' . $value['Key_name'] . '"');
...
//		$t_error_string = '';
		...
		{
			$this->clean_table($p_table, $p_columns_array);
		}
		$t_index_type = ($p_index_type == 'FULLTEXT') ? 'FULLTEXT' : 'BTREE';
		
		$t_index_names_array = $this->getIndicesNames($p_table, $t_non_unique, $t_index_type);
		$t_timestamp = time();
		
		if(sizeof($p_columns_array) <= 0)
		{
			//echo $this->set_index_error(4, $p_table, $p_index_type, $p_columns_array, $p_index_name);
			return false;
		}
		...
		if(!empty($p_index_name)){
			...
			if(in_array($p_index_name, $t_index_names_array))
			{
				...
				//echo $this->set_index_error(2, $p_table, $p_index_type, $p_columns_array, $p_index_name);
				return false;
			}
		}
		else
		{
			// no index to check
			//echo $this->set_index_error(1, $p_table, $p_index_type, $p_columns_array, $p_index_name);
			return false;
		}
		...
		$t_columns_exists = $this->check_col_exists($p_table, $p_columns_array);
		
		// 1 column or more dosn't exist
		if($t_columns_exists !== $p_columns_array)
		{
			//echo $this->set_index_error(3, $p_table, $p_index_type, $p_columns_array, $p_index_name);
			return false;
		}
		...
		{
			$t_sql = 'SHOW INDEX FROM ' . $this->coo_mysqli->real_escape_string($p_table) . ' WHERE Key_name = "PRIMARY"';
			$coo_result = $this->query($t_sql, true);
			if($coo_result->num_rows > 0)
			{
				$this->query('DROP INDEX `PRIMARY` ON ' . $this->coo_mysqli->real_escape_string($p_table));
			}
			$t_index_name_sql_part = '';
		}
		
		if(!$this->indexExists($p_table, $p_index_type, $p_columns_array))
		{
			$t_sql = "ALTER TABLE `" . $this->coo_mysqli->real_escape_string($p_table) . "`
