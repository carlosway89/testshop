    ...
    function process() {
      ...
      if (xtc_not_null($order->info['shipping_method'])) {
        ...
        // price with tax
          ...
          $shipping_cost_incl_tax = $xtPrice->xtcFormat(xtc_add_tax($order->info['shipping_cost'], $shipping_tax),false,0,false);
          $tax = round($shipping_cost_incl_tax - $order->info['shipping_cost'], 13);
          $tax = $xtPrice->xtcCalculateCurr($tax);
          $order->info['total'] -= round($order->info['shipping_cost'], 2);
