<?php
/* --------------------------------------------------------------
  MainFactory.inc.php 2015-09-07
  Gambio GmbH
  http://www.gambio.de
...
class MainFactory
{
	...
	{
		$coo_class_registry = self::$s_coo_class_registry_array[CURRENT_TEMPLATE];
		if($coo_class_registry === NULL)
		{
			# try building object from cache
			$t_cache_key = 'ClassRegistry_' . CURRENT_TEMPLATE;
			...
			self::$s_coo_class_registry_array[CURRENT_TEMPLATE] = $coo_class_registry;
		}
	...
	public static function create_object($p_class_name, $p_args_array = array(), $p_use_singleton = false)
	{
		...
		if($t_load_success == false)
		{
			...
			trigger_error('Class not found in registry: ' . CURRENT_TEMPLATE . ' # ' . $p_class_name, E_USER_ERROR);
			return false;
		}
