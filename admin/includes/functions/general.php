...
function check_stock($products_id)
{
	...
	{
		return '<i class="fa fa-check" title="' . $stock_values['products_quantity'] . ' ' . IMAGE_ICON_STATUS_GREEN_STOCK . '"></i>';
	}
}
...
function strip_slashes_from_url($p_value, $p_key)
{
	...
	{
		$t_return .= $p_key . '=' . stripslashes($p_value) . '&';
	}
...
function xtc_get_shipping_status()
{
	...
	$shipping_status_query = xtc_db_query("select shipping_status_id, shipping_status_name from " . TABLE_SHIPPING_STATUS . " where language_id = '" . $_SESSION['languages_id'] . "' order by number_of_days, shipping_status_id");
