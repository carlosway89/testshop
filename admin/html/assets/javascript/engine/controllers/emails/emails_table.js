...
gx.controllers.module(
	...
	[
		jse.core.config.get('shopUrl') + '/admin/html/assets/javascript/engine/libs/emails',
		'modal',
		'datatable',
		'normalize'
	],
	...
	function (data) {
		...
		var
			...
			defaults = {
				emailsTableActions: function () {
					return '<div class="row-actions">' + '<span class="send-email action-item" title="' + jse.core.lang.translate(
							'send',
							'buttons') + '">' + '<i class="fa fa-envelope-o"></i>' + '</span>' +
					       '<span class="forward-email action-item" title="' + jse.core.lang.translate('forward', 'buttons') +
					       '">' +
					       ...
					       '<span class="preview-email action-item" title="' + jse.core.lang.translate('preview', 'buttons') +
					       '">' +
					       '<i class="fa fa-eye"></i>' + '</span>' + '</div>';
				},

				convertPendingToString: function (data, type, row, meta) {
					return (data === true) ? jse.core.lang.translate('email_pending', 'emails') : jse.core.lang.translate(
						'email_sent', 'emails');
				}
			},
		...
		module.init = function (done) {
			...
			jse.libs.datatable.create($this, {
				...
				ajax: {
					url: jse.core.config.get('shopUrl') + '/admin/admin.php?do=Emails/DataTable',
					type: 'POST'
				},
				order: [[2, 'desc']],
				pageLength: 20,
