<?php
/* --------------------------------------------------------------
   DBBackupControl.inc.php 2014-11-12 tt@gambio
   Gambio GmbH
   http://www.gambio.de
...
class DBBackupControl
{
	...
	{	
		...
		$t_count_values = mysql_num_rows( $t_result );	
		while( $t_row = mysql_fetch_array( $t_result ) )
		{
			...
			while( $c_file_index < $t_count_files )
			{
				...
				else
				{
					...
					while( feof( $t_file ) === false )
					{
						...
						if( in_array( strtoupper_wrapper( strtok( $t_line, " " ) ), $this->v_sql_commands_list ) && $c_file_start_position != $c_actual_file_position )
						{
							// build sql command
							$t_query = trim( implode( "", $t_query ) );
							// execute sql command
