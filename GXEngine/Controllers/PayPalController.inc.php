<?php
/* --------------------------------------------------------------
	PayPalController.inc.php 2015-11-06
	Gambio GmbH
	http://www.gambio.de
...
class PayPalController extends HttpViewController
{
	...
	public function actionPrepareECS()
	{
		...
		try {
			...
			return new RedirectHttpControllerResponse($approval_url_href);
		}
		catch(Exception $e)
		{
			$this->logger->notice('Error creating ECS payment: '.$e->getMessage());
			return new RedirectHttpControllerResponse(xtc_href_link('shopping_cart.php', '', 'SSL'));
		}
	}
	...
	public function actionReturnFromECS()
	{
		...
		try {
			...
			if(!isset($_SESSION['customer_id']) || $_SESSION['customer_id'] == 0)
			{
				...
				{
					$countryId = MainFactory::create('Id', array(STORE_COUNTRY));
				}
				...
				try
				{
					...
					catch(UnexpectedValueException $e)
					{
						...
						else
						{
							...
							return new RedirectHttpControllerResponse(xtc_href_link('login.php', 'checkout_started=1', 'SSL'));
						}
					}
			...
			return new RedirectHttpControllerResponse(xtc_href_link('checkout_shipping.php', '', 'SSL'));
		}
		catch(Exception $e)
		{
			throw $e;
	...
	protected function getCustomerCountryIdByIso2($iso2)
	{
		...
		$id = new Id($query->row()->countries_id);
		return $id;
	}
	...
	protected function getCustomerByEmail($email_address)
	{
		...
		$customerId = new Id($query->row()->customers_id);
		$customerService = StaticGXCoreLoader::getService('Customer');
	...
	public function actionLogoffECSCustomer()
	{
		...
		return new HttpControllerResponse('logged_off');
	}
	...
	public function actionSetPayment()
	{
		...
		return new RedirectHttpControllerResponse(xtc_href_link('checkout_confirmation.php', '', 'SSL'));
	}
	...
	public function actionWebhook()
	{
		...
		return new HttpControllerResponse('OK');
	}
	...
	*/
	...
	public function actionPaylinkReturn()
	{
		...
		{
			return new HttpControllerResponse('required parameter(s) missing');
		}
		...
		if(class_exists('LayoutContentControl'))
		{
			...
			{
				return new HttpControllerResponse($layoutControl->get_response());
			}
		}
		else
		{
			return new HttpControllerResponse($mainContent);
		}
	}
