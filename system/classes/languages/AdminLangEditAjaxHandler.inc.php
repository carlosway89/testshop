<?php
/* --------------------------------------------------------------
   AdminLangEditAjaxHandler.inc.php 2015-04-06 gambio
   Gambio GmbH
   http://www.gambio.de
...
class AdminLangEditAjaxHandler extends AjaxHandler
{
	...
	public function proceed()
	{
		...
		if(isset($this->v_data_array['POST']['languages'])
		   && count($this->v_data_array['POST']['languages']) < count($languagesArray)
		)
		{
			$languagesFilterQuery = 'language_id IN (' . implode(',', $this->v_data_array['POST']['languages'])
			                        . ') AND ';
		}
