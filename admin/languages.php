...
switch($_GET['action'])
{
	...
	case 'copy':
		if($_SESSION['coo_page_token']->is_valid($_POST['page_token']))
		{
			...
			echo $query = 'INSERT INTO `email_templates_edited` (`name`, `language_id`, `type`, `content`, `backup`)
			...
			<tr>
				...
				<td class="boxCenter" width="100%" valign="top">
					...
					<table border="0" width="100%" cellspacing="0" cellpadding="2">
						<tr>
							<td width="100%">
								<div class="pageHeading" style="background-image:url(images/gm_icons/land.png)">
			...
			switch($_GET['action'])
			{
				case 'new':
					...
					$t_mode_selection_html .= '<script type="text/javascript">
																			...
																			{
																				$(\'#language_form input[type="radio"]\').change(function()
																				{
																					if($(this).val() == "insert" && $(this).prop("checked") == true)
																					{
																						$("#language_form").attr("action", $("#language_form").attr("action").replace("action=copy", "action=insert"));
																					}
																					else if($(this).val() == "copy" && $(this).prop("checked") == true)
																					{
																						$("#language_form").attr("action", $("#language_form").attr("action").replace("action=insert", "action=copy"));
																					}
																				});
																			});
																			</script>';
