<?php
/* --------------------------------------------------------------
  CachedDirectory.inc.php 2015-09-07
  Gambio GmbH
  http://www.gambio.de
...
class CachedDirectory
{
	...
	function CachedDirectory($p_path)
	{
		...
		if(!isset(self::$s_config_cache_paths_array[CURRENT_TEMPLATE]))
		{
			...
			self::$s_config_cache_paths_array[CURRENT_TEMPLATE] = $t_cache_paths_array;
			...
			foreach($t_templates_paths_array AS $t_path)
			{
				if(is_dir($t_path))
				{
					self::$s_config_cache_paths_array[CURRENT_TEMPLATE][] = $t_path;
				}
			}
		}
	...
	function set_cache_key($p_filename)
	{
		...
		{
			$this->v_cache_key = basename($p_filename . '_' . CURRENT_TEMPLATE);
	...
	{
		foreach(self::$s_config_cache_paths_array[CURRENT_TEMPLATE] AS $t_path)
		{
