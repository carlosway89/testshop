...
class ShopgatePluginGambioGX extends ShopgatePlugin
{
    ...
    private function getShipping(
        ...
        global $xtPrice, $total_count, $shipping_weight, $total_weight, $shipping_num_boxes, $order, $cart, $sendto, $billto;
        ...
        if (!empty($sgDeliverAddress)) {
            $countryId                                =
                $locationModel->getCountryByIso2Name($sgDeliverAddress->getCountry());
            $zone                                     = $locationModel->getZoneByCountryId($countryId);
            $order->delivery['country']['iso_code_2'] = $sgDeliverAddress->getCountry();
            $order->delivery['country']['id']         = $countryId;
            $order->delivery['zone_id']               = $zone['zone_id'];
            $_SESSION['delivery_zone']                = $sgDeliverAddress->getCountry();
        }
    ...
    private function insertOrderItems(ShopgateOrder $order, $dbOrderId, &$currentOrderStatus)
    {
        ...
        foreach ($order->getItems() as $orderItem) {
            ...
            $qry = xtc_db_query(
                "SELECT p.*, pd.products_name FROM " . TABLE_PRODUCTS . " p"
            ...
            $productData = array(
                "orders_id"              => $dbOrderId,
                "products_model"         => $orderItem->getItemNumberPublic(),
    ...
    public function getOrders(
        ...
        $customerOrderModel = new ShopgateCustomerOrderModel($customerToken);
