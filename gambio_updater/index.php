...
$t_content = 'language';
if(isset($_GET['content'])) $t_content = $_GET['content'];

$t_language = 'german';
if(isset($_GET['language']) && file_exists('lang/' . basename($_GET['language']) . '.inc.php')) $t_language = basename($_GET['language']);
...
{
	$t_check = $coo_db->query('SELECT * FROM `version_history` WHERE `version` = "2.1.3.0" AND `is_full_version` = "1"', true);
...
{
	$t_sql ='SELECT'
	        . ' main.`language_section_phrase_id`'
	        . ' FROM'
	        . ' (SELECT'
	        . ' `language_section_phrase_id`,'
	        . ' `language_section_id`,'
	        . ' `phrase_name`,'
	        . ' COUNT(*) AS variants'
	        . ' FROM'
	        ...
	        . ' ORDER BY'
	        . ' NULL) AS sub,'
	        . ' `language_section_phrases` AS main'
	        . ' WHERE'
	...
	if(count($t_check) > 0)
	{
		...
		$t_sql = 'DELETE FROM `language_section_phrases` WHERE `language_section_phrase_id` IN (' . implode(',', $t_language_section_phrase_id_array) . ')';
		$coo_db->query($t_sql);
	}
	...
	{
		$coo_db->set_index('language_section_phrases', 'UNIQUE', array('language_section_id', 'phrase_name'), 'language_section_phrase');
	}
}
...
if((isset($_GET['action']) && $_GET['action'] == 'login') || isset($_GET['content']) && in_array($_GET['content'], array('setup', 'configure', 'move', 'chmod', 'delete_files', 'finish')))
{
	if(isset($_POST['email']) && isset($_POST['password']) && $coo_update_control->login($_POST['email'], $_POST['password']))
	{
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript" src="javascript/main.js.php?language=<?php echo $t_language . '&rand=' . time(); ?>"></script>
	</head>
		...
		<div id="main" class="gradient">
			...
			<div id="install_service">
				...
				<p>
					...
					<br />
					<a href="http://www.gambio-support.de" class="button gradient red" target="_blank"><?php echo BUTTON_GAMBIO_PORTAL; ?></a>
				</p>
			...
			switch($t_content)
			{
				case 'move':
					...
					$t_update_dir_array = unserialize(file_get_contents(DIR_FS_CATALOG . 'cache/update_dir_array.pdc'));
					...
					{
						$t_move_form = '<form id="form_move" action="index.php?content=finish&language=' . $t_language . '" method="post" autocomplete="off">' . $t_move_form;
					}
					elseif(empty($t_move_array) && empty($t_delete_array) === false)
					{
						$t_move_form = '<form id="form_move" action="index.php?content=delete_files&language=' . $t_language . '" method="post" autocomplete="off">' . $t_move_form;
					}
					elseif(empty($t_move_array))
					{
						$t_move_form = '<form id="form_move" action="index.php?content=chmod&language=' . $t_language . '" method="post" autocomplete="off">' . $t_move_form;
					}
					else
					{
						$t_move_form = '<form id="form_move" action="index.php?content=move&language=' . $t_language . '" method="post" autocomplete="off">' . $t_move_form;
					}
					
					$t_move_form .= '<input type="hidden" name="email" value="' . htmlspecialchars($t_email, ENT_COMPAT, 'UTF-8') . '" />';
					$t_move_form .= '<input type="hidden" name="password" value="' . htmlspecialchars($t_password, ENT_COMPAT, 'UTF-8') . '" />';
					$t_move_form .= '</form>';
				...
				case 'delete_files':
					...
					{
						$t_additional_delete_list = unserialize(file_get_contents(DIR_FS_CATALOG . 'cache/additional_delete_list.pdc'));
						clearstatcache();
					...
					{
						$t_delete_form = '<form id="form_delete" action="index.php?content=finish&language=' . $t_language . '" method="post" autocomplete="off">' . $t_delete_form;
					}
					else if (empty($t_delete_array))
					{
						$t_delete_form = '<form id="form_delete" action="index.php?content=chmod&language=' . $t_language . '" method="post" autocomplete="off">' . $t_delete_form;
					}
					else
					{
						$t_delete_form = '<form id="form_delete" action="index.php?content=delete_files&language=' . $t_language . '" method="post" autocomplete="off">' . $t_delete_form;
					}
					$t_delete_form .= '<input type="hidden" name="email" value="' . htmlspecialchars($t_email, ENT_COMPAT, 'UTF-8') . '" />';
					$t_delete_form .= '<input type="hidden" name="password" value="' . htmlspecialchars($t_password, ENT_COMPAT, 'UTF-8') . '" />';
					$t_delete_form .= '</form>';
				...
				case 'chmod':
					...
					{
						$t_chmod_form = '<form id="form_chmod" action="index.php?content=finish&language=' . $t_language . '" method="post" autocomplete="off">' . $t_chmod_form;
					}
					else
					{
						$t_chmod_form = '<form id="form_chmod" action="index.php?content=chmod&language=' . $t_language . '" method="post" autocomplete="off">' . $t_chmod_form;
					}
					
					$t_chmod_form .= '<input type="hidden" name="email" value="' . htmlspecialchars($t_email, ENT_COMPAT, 'UTF-8') . '" />';
					$t_chmod_form .= '<input type="hidden" name="password" value="' . htmlspecialchars($t_password, ENT_COMPAT, 'UTF-8') . '" />';
					$t_chmod_form .= '</form>';
				...
				case 'configure':
					...
					file_put_contents(DIR_FS_CATALOG . 'cache/update_dir_array.pdc', serialize($t_update_dir_array));
					...
					echo '<br /><input id="button_install" type="button" name="proceed" value="' . BUTTON_INSTALL . '" class="button green" />';
					echo '<input type="hidden" name="email" value="' . htmlspecialchars($t_email, ENT_COMPAT, 'UTF-8') . '" />';					
					echo '<input type="hidden" name="password" value="' . htmlspecialchars($t_password, ENT_COMPAT, 'UTF-8') . '" />';
					echo '</form>';
					
					echo '<div id="update_status">';
					echo '<p><strong>' . HEADING_PROGRESS . '</strong></p>
							<p>' . TEXT_PROGRESS . '<span id="current_update"></span> <img src="images/processing.gif" width="16" height="16" /></p>';	
					echo '</div>';
				...
				case 'login':
					...
					echo '		<table width="620" border="0" cellspacing="5" cellpadding="0">
									<tr>
										<td>' . LABEL_EMAIL . '</td>
										<td><input type="text" class="input_field" name="email" size="35" value="' . htmlspecialchars($t_email, ENT_COMPAT, 'UTF-8') . '" /></td>
									</tr>
									<tr>
...
<input type="submit" class="button red" name="force_version_selection" value="' . LABEL_FORCE_VERSION_SELECTION_BTN . '" />
<br style="clear: both;" />
<br/>
...
<br/>';
					...
					if ($coo_update_control->current_db_version === false && !isset($_POST['shop_version']) || isset($_POST['force_version_selection']))
					{
						echo '<form id="form_setup" action="index.php?content=setup&language=' . $t_language . '" method="post">';
						...
						echo '<option value="2.1.3.0">v2.1.3</option>';
						echo '<option value="2.1.4.0">v2.1.4</option>';
						echo '<option value="2.1.5.0">v2.1.5</option>';
						echo '<option value="2.2.0.0">v2.2.0</option>';
						echo '<option value="2.2.1.0">v2.2.1 oder höher (v2.2.x)</option>';
						echo '<option value="2.3.1.0">v2.3.1 oder höher (v2.3.x)</option>';
						echo '<option value="2.4.0.0">v2.4.0 oder höher (v2.4.x)</option>';
						echo '<option value="2.5.1.0">v2.5.1</option>';
						echo '<option value="2.5.2.0">v2.5.2</option>';
						echo '<option value="2.5.3.0">v2.5.3</option>';
						echo '<option value="2.6.0.0">v2.6.0 oder höher (v2.6.x)</option>';
						echo '</select>';
						echo '		</td>
								</tr>
							</table><br />';
						echo '<input type="hidden" name="email" value="' . htmlspecialchars($t_email, ENT_COMPAT, 'UTF-8') . '" />';					
						echo '<input type="hidden" name="password" value="' . htmlspecialchars($t_password, ENT_COMPAT, 'UTF-8') . '" />';					
						echo '<input type="submit" class="button green" name="choose_version" value="' . BUTTON_SHOW_UPDATES . '" />';
					...
					else
					{
						...
						if (empty($t_delete_array) && empty($t_chmod_array))
						{
							echo '<form id="form_delete" action="index.php?content=configure&language=' . $t_language . '" method="post">';
						}
						...
						if(empty($coo_update_control->gambio_update_array) == false)
						{
							...
							foreach($coo_update_control->gambio_update_array as $key => $coo_update_model)
							{
								...
								$updatedFilesUploaded &=
									$requirementsTesting->testUpdateFiles($coo_update_model->get_update_name(), $newest);
								$styleEditFilesUploaded &=
									$requirementsTesting->testStyleEditFiles($coo_update_model->get_update_name());
								
								echo '- ' . $coo_update_model->get_update_name() . '<br />';
							}
							$info = $requirementsTesting->getInfo();
//var_dump($updatedFilesUploaded, $styleEditFilesUploaded);
							if(!$updatedFilesUploaded):
								echo '</p>';
								?>
								<div class="files-not-uploaded" style="word-wrap: break-word; margin-bottom: 30px">
									<h4 class="warning" style="text-align: center"><?php echo TEXT_NOT_ALL_FILES_UPLOADED; ?></h4>

									<button class="button"
									        type="button"
									        onclick="$('#not-uploaded-file-list').toggle()">Dateien anzeigen
									</button>
									<button class="button" type="button" onclick="location.reload()"><?php echo BUTTON_CHECK_MOVE ?></button>
									<div style="display: none; margin-top: 12px; max-height: 350px; overflow: auto"
									     id="not-uploaded-file-list">
										<?php
										foreach($info['updatedFiles'] as $file):
											echo $file . '<hr/>';
										endforeach;
										?>
									</div>
							...
							if(0 === count($info['updatedFiles'])):

								if(!$styleEditFilesUploaded):
									echo '</p>';
									?>
									<div class="files-not-uploaded" style="word-wrap: break-word; margin-bottom: 30px">
										<h4 class="warning" style="text-align: center"><?php echo TEXT_NOT_ALL_SE_V2_FILES_UPLOADED; ?></h4>
										...
										<div style="display: none; margin-top: 12px; max-height: 350px; overflow: auto"
										     id="not-uploaded-file-list">
											<?php
											foreach($info['styleEditV2Files'] as $file):
												echo $file . '<hr/>';
											endforeach;
											?>
										</div>
						...
						else
						{
							...
							{
								echo '<a href="' . HTTP_SERVER . DIR_WS_CATALOG . '" class="button green float_left">' . BUTTON_SHOP .
									 '</a>';
							}
							
							echo $t_force_version_selection_html;

							echo '</p><input type="hidden" name="email" value="' .
								 htmlspecialchars($t_email, ENT_COMPAT, 'UTF-8') . '" />';
							echo '<input type="hidden" name="password" value="' .
								 htmlspecialchars($t_password, ENT_COMPAT, 'UTF-8') . '" />';
			...
			<div id="result">
				...
				<div id="clear_cache">
					...
					<p><?php echo TEXT_INSTALLATION_CLEAR_CACHE; ?> <img src="images/processing.gif" width="16" height="16" /></p>
				</div>
				...
				if($hasUpdates)
				{
					$latestUpdate = $coo_update_control->gambio_update_array[count($coo_update_control->gambio_update_array) - 1];
				...
				<div id="installation_success">
					...
					<p>
						...
						if($hasUpdates && $coo_update_control->current_db_version < '2.3.1.0' && $latestVersion >= '2.3.1.0')
						{
				...
				<div id="sql_errors_report">
					<?php echo TEXT_SQL_ERRORS; ?><br />
					<textarea id="sql_errors" readonly="readonly"></textarea>
					<br /><br />
				</div>
				
				<div id="conflicts_report"><?php echo TEXT_SECTION_CONFLICT_REPORT; ?></div>
				
				<a href="<?php echo HTTP_SERVER . DIR_WS_CATALOG; ?>" class="button green"><?php echo BUTTON_SHOP; ?></a>
		...
		<div id="copyright">
			<strong><a href="http://www.gambio.de" target="_blank"><strong>Gambio.de</strong></a> - Gambio-Updater &copy; 2016 Gambio GmbH</strong><br />
