<?php
/* --------------------------------------------------------------
  GMSitemapXML.php  2014-06-21 gm
  Gambio GmbH
  http://www.gambio.de
...
class GMSitemapXML_ORIGIN
{
	...
	function create()
	{
		...
		$t_xml .= $this->get_categories();

		$t_xml .= $this->get_content();

		$t_xml .= "\n\t" . '</urlset>';

		$this->output($t_xml);

		return TITLE_SITEMAP_CREATED . $this->link_counter . TITLE_SITEMAP_CREATED2 . '<a href="' . HTTP_SERVER . DIR_WS_CATALOG . $this->filename . '" target="_blank">' . $this->filename . '</a><br /><br />';
	}
