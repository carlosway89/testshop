<?php
/* --------------------------------------------------------------
  gm_pdf_order.php 2015-09-28 gm
  Gambio GmbH
  http://www.gambio.de
...
if($_GET['mail'] == 1)
{
	...
	if(strstr($mail_text,'{SALUTATION}'))
	{
		if($order_check['customers_gender'] == 'm')
		{
			$mail_text = str_replace('{SALUTATION}', gm_get_content('GM_PDF_SALUTATION_MALE', $_SESSION['languages_id']), $mail_text);
		}
		else
		{
			$mail_text = str_replace('{SALUTATION}', gm_get_content('GM_PDF_SALUTATION_FEMALE', $_SESSION['languages_id']), $mail_text);
		}
	}
