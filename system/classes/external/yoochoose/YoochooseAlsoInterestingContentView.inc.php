...
class YoochooseAlsoInterestingContentView extends YoochooseProductView {
	...
	function get_html($product) {
		...
		if ($numrec > 0) {
			$yoo_recos = recommend(YOOCHOOSE_PRODUCT_ALSO_INTERESTING_STRATEGY, $product->pID, $numrec * 2);
			
			if (! isset($yoo_exclude_list)) {
				$yoo_exclude_list = array();
			}	
		
			$yoo_products = createBoxRecords(YOOCHOOSE_PRODUCT_ALSO_INTERESTING_STRATEGY, $product, $yoo_recos, $numrec, $yoo_exclude_list);
