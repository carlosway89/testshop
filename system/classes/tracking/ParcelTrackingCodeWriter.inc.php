<?php
/* --------------------------------------------------------------
   ParcelTrackingCodeWriter.inc.php 2014-10-08 gm
   Gambio GmbH
   http://www.gambio.de
...
class ParcelTrackingCodeWriter
{
	...
	{
		$query = 'DELETE FROM orders_parcel_tracking_codes WHERE orders_parcel_tracking_code_id = ' . (int)$p_trackingId;
	 ...
	 * @throws Exception
	 */
	public function insertTrackingCode($p_orderId, $p_trackingCode, $p_parcelServiceId, ParcelServiceReader $parcelServiceReadService)
	{
		if(empty($p_orderId))
		{
		...
		$comment = $this->resolveTrackingCodePlaceholder($parcelService->getCommentByLanguageId($languageId), $p_trackingCode);
	 ...
	 * @throws Exception
	 */
	public function insertTrackingUrl($p_orderId, $p_trackingUrl, $p_parcelServiceId, ParcelServiceReader $parcelServiceReadService)
	{
		if(empty($p_orderId))
		{
			throw new UnexpectedValueException('cannot insert tracking code, because orderId empty');
		...
		$query = 'INSERT INTO orders_parcel_tracking_codes
					SET
						order_id = ' . (int)$p_orderId . ',
						tracking_code = "' . xtc_db_input($url) . '",
						parcel_service_id = ' . (int)$p_parcelServiceId . ',
						parcel_service_name = "' . xtc_db_input($parcelServiceName) . '",
						language_id = ' . (int)$languageId . ',
						url = "' . xtc_db_input($url) . '",
