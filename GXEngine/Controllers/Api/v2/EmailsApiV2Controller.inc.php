<?php
/* --------------------------------------------------------------
   EmailsApiV2Controller.inc.php 2015-07-08 gm
   Gambio GmbH
   http://www.gambio.de
...
class EmailsApiV2Controller extends HttpApiV2Controller
{
	 ...
	 * then they must already exist in the server. You will need to provide the full path to the file.
	 *
	 ...
	 * }
	 *
	 * @apiParam {int} id (optional) If provided then an existing email will be resend (only applies to URL).
	 * @apiParam {string} subject (optional) Email subject to be sent.
	 * @apiParam {object} sender Contains the sender contact data.
	 * @apiParam {string} sender.emailAddress Sender's email address.
	 * @apiParam {string} sender.contactName (optional) Sender display name.
	 ...
	 * @apiParam {string} recipient.contactName (optional) Recipient's display name.
	 * @apiParam {object} replyTo Contains the reply to contact data.
	 * @apiParam {string} replyTo.emailAddress Email subject to be sent.
	 * @apiParam {string} replyTo.contactName Email subject to be sent.
	 * @apiParam {string} contentHtml (optional) Email subject to be sent.
	 * @apiParam {string} contentPlain (optional) Email subject to be sent.
	 * @apiParam {array} bcc (optional) Contains the BCC contacts of the email.
	 * @apiParam {array} cc (optional) Contains the CC contacts of the email.
	 * @apiParam {array} attachments (optional) Contains the attachment data.
	 * @apiParam {string} attachments[].path The path to the attachments (the file must already exist in the server).
	 * @apiParam {string} attachments[].name (optional) Set a display name for the attachment file (must also contain
	 *           the file extension).
	 *
	 * @apiSuccess (Success 200 - Email Was Sent) {string} Response-Body If successful, this
	...
	public function post()
	{
		...
		{
			$email = $this->emailService->findById(MainFactory::create('Id', (int)$this->uri[1]));
		...
		else 
		{
			...
			{
				$baseObject = $this->emailService->findById(MainFactory::create('Id',(int)$this->uri[1]));
	 ...
	 * the "post" method for parameter description.
	 *
	 ...
	 * @apiGroup   Emails
	 *
	 * @apiParam {int} id Must be the ID of the referenced email record. If missing the method will
	 * return a 400 Bad Request status code.
	 *
	 * @apiSuccess (Success 200 - Email Was Deleted) {string} Response-Body If successful this
	 * method returns information about the deleted record.
	 *
	...
	public function delete()
	{
		...
		$id    = (int)$this->uri[1];
		$email = $this->emailService->getById(MainFactory::create('Id', $id));
	 ...
	 *
	 * @apiParam {int} id (Optional) Email record id to be returned. If omitted all the email records
	...
	public function get()
	{
		...
		else if($this->api->request->get('state') !== null)
		{
