...
//			foreach($this->v_styles as $t_style)
			while($t_style = mysql_fetch_array($t_result))
			{
				$t_result_content = mysql_query('
												SELECT
				...
				while(($t_row_content = mysql_fetch_array($t_result_content) ))
				{
			...
			if($this->v_current_template == 'gambio')
			{
				...
				$t_get_underline_setting = mysql_query("SELECT gm_value
																		FROM gm_configuration
																		WHERE
																			gm_key = 'GM_UNDERLINE_LINKS'
																			AND gm_value = 'true'");
				if(mysql_num_rows($t_get_underline_setting) > 0)
				{
...
//			foreach($this->v_styles as $t_style)
		...
		*/
		...
		{
			$c_key = mysql_real_escape_string($p_key);
			$t_value = '';

			$t_query = mysql_query("SELECT gm_value
									FROM gm_configuration
									WHERE
										gm_key = '" . $c_key . "'
										LIMIT 1");
			if(mysql_num_rows($t_query) == 1)
			{
				$t_result_array = mysql_fetch_array($t_query);
