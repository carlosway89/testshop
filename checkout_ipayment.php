<?php
/* --------------------------------------------------------------
   checkout_ipayment.php 2014-03-26 mabr
   Gambio GmbH
   http://www.gambio.de
...
if($_SERVER['REQUEST_METHOD'] == 'GET') {
	...
	if(isset($_REQUEST['ret_errormsg'])) {
		$smarty->assign('silent_error', utf8_encode(strip_tags($_REQUEST['ret_errormsg'])));
	}

	$smarty->assign('formfields', $formfields);
	$smarty->assign('cards_enabled', $cards_enabled);
	...
	$smarty->assign('back_url', GM_HTTP_SERVER.DIR_WS_CATALOG.basename(__FILE__).'?back_button=go');
	//$smarty->assign('debug', '<pre>'.$debug.htmlspecialchars(print_r($formfields, true).print_r($ipay_order, true).print_r($_REQUEST, true)).'</pre>');
