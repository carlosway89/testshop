<?php
/* --------------------------------------------------------------
  CSVAjaxHandler.inc.php 2014-01-03 gm
  Gambio GmbH
  http://www.gambio.de
			...
			else if($_SESSION['customers_status']['customers_status_id'] === '0')
			{
				switch($t_action_request)
				{
					...
					case 'upload_import_file':
						...
						break;
					case 'import':
						if(isset($this->v_data_array['POST']['filename']) && isset($this->v_data_array['POST']['separator']))
						{
							$t_filename = $this->v_data_array['POST']['filename'];
							$t_separator = $this->v_data_array['POST']['separator'];
							$t_quote = $this->v_data_array['POST']['quote'];
							$t_quote = str_replace('\"', '"', $t_quote);
							$t_quote = str_replace("\'", "'", $t_quote);
							$t_progress = $this->v_data_array['POST']['progress'];
							$t_output_array = $coo_csv_control->import($t_filename, $t_separator, $t_quote, $t_progress);
						}
						break;
