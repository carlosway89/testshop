<?php
/* --------------------------------------------------------------
  class.skrill.php 2014-07-15 gm
  Gambio GmbH
  http://www.gambio.de
...
class fcnt_skrill_ORIGIN {
	...
	function _setCode($code='CC',$payment_method='ACC') {
		...
		if (defined('MODULE_PAYMENT_SKRILL_'.strtoupper($code).'_TEXT_TITLE')) {
			...
			$this->description .= constant('MODULE_PAYMENT_SKRILL_'.strtoupper($code).'_NOTE_CONFIGURATION');
			$this->description = strtr($this->description, array(':url_skrill_config' => DIR_WS_ADMIN.'configuration.php?gID=32'));
