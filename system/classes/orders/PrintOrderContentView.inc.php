<?php
/* --------------------------------------------------------------
  PrintOrderContentView.inc.php 2014-04-24 gm
  http://www.gambio.de
...
class PrintOrderContentView extends ContentView
{
	...
	protected function add_order_data()
	{
		...
		{
			$this->coo_language_text_manager->init_from_lang_file('lang/' . $this->language . '/modules/payment/' . $this->coo_order->info['payment_method'] . '.php');
			$t_payment_method = constant(strtoupper('MODULE_PAYMENT_' . $this->coo_order->info['payment_method'] . '_TEXT_TITLE'));
