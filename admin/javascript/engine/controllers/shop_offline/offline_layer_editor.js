/* --------------------------------------------------------------
 offline_layer_editor.js 2014-09-29 gm
 Gambio GmbH
 http://www.gambio.de
	...
	function (data) {
		...
		var _alterNames = function (revert) {
			$fields.each(function () {
				...
				name = (revert) ? (name.replace(appendName, 'switcher checked')) : (name + appendName);
				$self.attr('name', name);
			});
		};
		...
		module.init = function (done) {
			...
			var $layer = $('#lightbox_package_' + lightbox_parameters.identifier),
				...
				dataset = jse.libs.form.getData($parentForm);
			...
			jse.core.fallback.setupWidgetAttr($this);
			...
			$layer.on('click', '.ok', function () {
				...
				_alterNames(true);
				jse.libs.form.prefillForm($parentForm, jse.libs.form.getData($form), false);
			...
			$layer.on('widget.initialized', function (e, d) {
