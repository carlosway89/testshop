<?php
/* --------------------------------------------------------------
   create_account.php 2015-10-19 gm
   Gambio GmbH
   http://www.gambio.de
...
if ($_GET['action'] == 'edit' && !empty($_POST)) {
	...
	if (strlen_wrapper($customers_firstname) < ENTRY_FIRST_NAME_MIN_LENGTH) {
		$error = true;
	...
	if ($error == false) {
		...
		$addressBlock = MainFactory::create('AddressBlock', 
			...
			$countryService->getCountryById(MainFactory::create('Id', $entry_country_id)),
			$countryZone
		);

		$dateOfBirth = MainFactory::create('CustomerDateOfBirth', '0000-00-00');
		if(ACCOUNT_DOB == 'true')
		{
		...
		<tr class="no-hover">
			...
			<td class="boxCenter" width="100%" valign="top">
				<div class="pageHeading"
				     style="background-image:url(images/gm_icons/kunden.png)"><?php echo HEADING_TITLE; ?></div>
