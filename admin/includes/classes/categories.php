...
// remove_category ends
	...
	function insert_category($categories_data, $dest_category_id, $action = 'insert')
	{
		...
		if(!empty($_FILES['categories_icon']['tmp_name']))
		{
			...
			if($categories_icon_name && $categories_icon_name[1] !== false && file_exists(DIR_FS_CATALOG . 'images/categories/icons/' . $categories_icon_name[1]))
			{
				// unlink old file
		...
		if(!(in_array($src_category_id, $_SESSION['copied'])))
		{
			...
			while($cdcopy_values = xtc_db_fetch_array($cdcopy_query))
			{
				...
				if(xtc_db_num_rows($t_gm_check) == 0)
				{
					...
					$t_sql_data['categories_name'] = addslashes($cdcopy_values['categories_name']);
...
// delete_product ends
	...
	function insert_product($products_data, $dest_category_id, $action = 'insert')
	{
		...
		else
		{
			...
			if(!is_writable(DIR_FS_CATALOG . 'images/product_images/original_images') && isset($_FILES['products_image']))
			{
		...
		if(xtc_not_null($_FILES))
		{
			foreach($_FILES as $key => $file)
			{
				...
				elseif(!is_writable(DIR_FS_CATALOG . 'images/product_images/original_images'))
				{
					$error_message = sprintf(ERROR_DIRECTORY_NOT_WRITEABLE, 'images/product_images/original_images');
					$GLOBALS['messageStack']->add_session($error_message, 'error');
				}
	...
	function duplicate_product($src_products_id, $dest_categories_id)
	{
		...
		while($t_item_codes_result_array = xtc_db_fetch_array($t_item_codes_result))
		{
			...
			$t_item_codes_data_array = array(
				...
				'identifier_exists' => mysql_real_escape_string($t_item_codes_result_array['identifier_exists']),
				'gender' => mysql_real_escape_string($t_item_codes_result_array['gender']),
				'age_group' => mysql_real_escape_string($t_item_codes_result_array['age_group']),
				'expiration_date' => mysql_real_escape_string($t_item_codes_result_array['expiration_date']),
				'products_id' => (int)$dup_products_id
			);
