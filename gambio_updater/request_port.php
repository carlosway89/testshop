<?php
/* --------------------------------------------------------------
   request_port.php 2015-07-12 gm
   Gambio GmbH
   http://www.gambio.de
...
$t_language = isset($_GET['language']) && file_exists('lang/' . basename($_GET['language']) . '.inc.php') ? basename($_GET['language']) : 'german';
...
if($coo_update_control->login($_POST['email'], $_POST['password']))
{
	...
	if($t_action == 'ftp' && $t_content == 'move' || $t_content == 'delete' || $t_content == 'chmod')
	{
		...
		if(isset($_POST['FTP_HOST']) && isset($_POST['FTP_USER']) && isset($_POST['FTP_PASSWORD']) && isset($_POST['FTP_PASV']))
		{
	...
	else
	{
		...
		if(isset($_GET['current_update'])) $t_current_update = $_GET['current_update'];

		debug_notice('request_port.php: action \'' . $t_action . '\', step \'' . $t_step . '\', current_update \'' . $t_current_update . '\'');
		...
		switch($t_action)
		{
			...
			default:
				...
				{
					$t_additional_delete_list_array = array_merge($t_additional_delete_list_array, $coo_update->get_section_files_array(true));
				}
				...
				{
					file_put_contents(DIR_FS_CATALOG . 'cache/additional_delete_list.pdc', serialize($t_additional_delete_list_array));
				}
				
				$t_success = $coo_update_control->update($t_current_update, $t_refusion_array, $t_execute_dependent_queries, $t_execute_independent_queries, $t_update_css, $t_update_sections, $t_update_version_history);
				if($t_success)
				{
					...
					if($t_update_version_history == true && $t_next_step != $t_step)
					{
						...
						foreach($coo_update_control->gambio_update_array AS $t_key => $coo_update_model)
						{
							if($coo_update_model->get_update_name() == $t_current_update)
							{
								...
								{
									$t_next_update = $coo_update_control->gambio_update_array[$t_key+1]->get_update_name();
								}
					...
					if($t_next_update != '')
					{
						$t_output_array['url'] = 'request_port.php?action=install&language=' . $t_language . '&current_update=' . rawurlencode($t_next_update) . '&step=' . $t_next_step;
			...
			else
			{
				if(!$t_success)
				{
					...
					$error_msg = str_replace('x.x.x.x', $coo_update->get_update_name(), ERROR_SQL_UNKNOWN);
