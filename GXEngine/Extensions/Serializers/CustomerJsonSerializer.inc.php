<?php
/* --------------------------------------------------------------
   CustomerJsonSerializer.inc.php 2015-07-08 gm
   Gambio GmbH
   http://www.gambio.de
...
class CustomerJsonSerializer extends AbstractJsonSerializer
{
	...
	public function deserialize($string, $baseObject = null)
	{
		...
		if($json->id !== null)
		{
			$customer->setId(MainFactory::create('Id', $json->id));
		}
		...
		if($json->addressId !== null)
		{
			...
			$address        = $addressService->findAddressById(MainFactory::create('Id', (int)$json->addressId));
