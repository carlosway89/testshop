/* --------------------------------------------------------------
 language_switcher.js 2015-10-16 gm
 Gambio GmbH
 http://www.gambio.de
...
gx.extensions.module(
	...
	/** @lends module:Extensions/language_switcher */
	...
	function (data) {
		...
		var _generateTransferObject = function (langActive, toHidden) {
			...
			var currentData = {},
				fullData = jse.core.fallback.getData($this);
		...
		module.init = function (done) {
			...
			$this.on('widget.initialized', function(e, widgetName) {
				if (widgetName === 'ckeditor') {
					_updateCKeditors();
				}
			});
			...
			$this
				.on('widget.initialized', function (e, d) {
					// Update ckeditors if an widget is initialized inside $this.
					if (d.widget !== 'language_switcher') {
						_updateCKeditors();
					}
				})
