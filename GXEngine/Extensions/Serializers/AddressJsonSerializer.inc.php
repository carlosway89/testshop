<?php
/* --------------------------------------------------------------
   AddressJsonSerializer.inc.php 2015-07-08 gm
   Gambio GmbH
   http://www.gambio.de
...
class AddressJsonSerializer extends AbstractJsonSerializer
{
	...
	public function serialize($object, $encode = true)
	{
		...
		$address = array(
				...
				'customerId' => ($object->getCustomerId() !== null) ? (int)(string)$object->getCustomerId() : null,
				...
				'countryId'  => ($object->getCountry() !== null) ? (int)(string)$object->getCountry()->getId() : null,
				'zoneId'     => ($object->getCountryZone() !== null) ? (int)(string)$object->getCountryZone()
				                                                                           ->getId() : null,
				'class'      => ($object->getAddressClass() !== null) ? (string)$object->getAddressClass() : null,
				'b2bStatus'  => ($object->getB2BStatus() !== null) ? (bool)(string)$object->getB2BStatus() : null,
		);
	...
	public function deserialize($string, $baseObject = null)
	{
		...
		if($json->id !== null)
		{
			$address->setId(MainFactory::create('Id', (int)$json->id));
		}

		if($json->customerId !== null)
		{
			$address->setCustomerId(MainFactory::create('Id', (int)$json->customerId));
		}
		...
		{
			$country = $countryService->getCountryById(MainFactory::create('Id', (int)$json->countryId));
			$address->setCountry($country);
		}

		if($json->zoneId !== null)
		{
			$zone = $countryService->getCountryZoneById(MainFactory::create('Id', (int)$json->zoneId));
