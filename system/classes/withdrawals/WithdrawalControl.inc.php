<?php
/* --------------------------------------------------------------
  WithdrawalControl.inc.php 2015-12-03 gambio
  Gambio GmbH
  http://www.gambio.de
...
class WithdrawalControl extends DataProcessing
{
	...
	protected function send_confirmation_mail(WithdrawalModel $p_coo_withdrawal, $p_email)
	{
		...
		$t_html = $coo_withdrawal_confirmation_view->get_html('html', $_SESSION['languages_id'], $_SESSION['language']);

		$t_txt = $coo_withdrawal_confirmation_view->get_html('txt', $_SESSION['languages_id'], $_SESSION['language']);
		...
		{
			// send mail to admin
			xtc_php_mail($p_email, $p_coo_withdrawal->get_customer_name(), EMAIL_BILLING_ADDRESS, STORE_NAME, EMAIL_BILLING_FORWARDING_STRING, $p_email, $p_coo_withdrawal->get_customer_name(), '', '', $t_subject, $t_html, $t_txt);
