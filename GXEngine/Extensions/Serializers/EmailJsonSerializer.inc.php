<?php
/* --------------------------------------------------------------
   EmailsJsonSerializer.inc.php 2015-07-08 gm
   Gambio GmbH
   http://www.gambio.de
...
class EmailJsonSerializer extends AbstractJsonSerializer
{
	...
	public function deserialize($string, $baseObject = null)
	{
		...
		if($json->id !== null)
		{
			$email->setId(MainFactory::create('Id', (int)$json->id));
		}
		...
		{
			$email->setCreationDate(new DateTime($json->creationDate));
		}

		if($json->sentDate !== null)
		{
			$email->setSentDate(new DateTime($json->sentDate));
		}
