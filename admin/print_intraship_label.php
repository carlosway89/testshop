<?php
/* -----------------------------------------------------------------------------------------
$Id: print_intraship_label.php v1.00 2015-09-28 gm
...
if(isset($_GET['WeightInKG'])) {
	$dhl_weight = preg_replace('/[^0-9.,]/','',$_GET['WeightInKG']);
...
if(((isset($_GET['getlabel'])) || (isset($_GET['stornolabel']))) && ($_GET['error'] == '')) {
	...
	try {
		...
		if($statuscheck->StatusCode != '0') {
			...
			xtc_redirect(xtc_href_link('print_intraship_label.php', 'oID='.$oID.'&error=1&function='.$function));
		}
		else {
	...
	if(isset($_GET['getlabel'])) {
		...
		//orders.php über Formular updaten und Label-PDF anzeigen
		echo "<html>";
		echo "<head>";
		echo "<title>DHL Intraship</title>";
		echo "</head>";
		echo "<body>";
		echo '<form action="orders.php?page=1&oID='.$oID.'&action=update_order" method="post" name="formular">';
		echo '<input type="hidden" name="status" value="'.$gmintraship->status_id_sent.'">';
		...
		if($gmintraship->send_email == true) {
			echo '<input type="hidden" name="notify" value="on">';
			echo '<input type="hidden" name="notify_comments" value="on">';
		}
		else {
			echo '<input type="hidden" name="notify" value="off">';
			echo '<input type="hidden" name="notify_comments" value="off">';
		}
		if(is_pageTokenClassAvailable() == true) {
			echo '<input type="hidden" name="page_token" value="' . $page_token . '">';
		}
		echo "</form>\n";
		echo "<script language=\"javascript\">\n";
		echo "window.open(\"".$result->CreationState->Labelurl."\",\"INTRASHIP\",\"height=600,width=640,resizable=1\");\n";
		echo "document.forms.formular.submit();\n";
		echo "</script>\n";
		echo "</body>";
		echo "</html>";
		die;
	...
	if(isset($_GET['stornolabel'])) {
		...
		//orders.php über Formular updaten
		echo "<html>";
		echo "<head>";
		echo "<title>DHL Intraship</title>";
		echo "</head>";
		echo "<body>";
		echo '<form action="orders.php?oID='.$oID.'&action=update_order" method="post" name="formular">';
		echo '<input type="hidden" name="status" value="'.$gmintraship->status_id_storno.'">';
		echo '<input type="hidden" name="comments" value="'.$gmintraship->get_text('CANCELTEXT').'">';
		if($gmintraship->send_email == true) {
			echo '<input type="hidden" name="notify" value="on">';
			echo '<input type="hidden" name="notify_comments" value="on">';
		}
	...
	<table border="0" width="100%" cellspacing="2" cellpadding="2">
		<tr>
			...
			<td  class="boxCenter" width="100%" valign="top">
				...
				<?php
					...
					if($_GET['error'] != '') {
						...
						if(isset($_REQUEST['debug'])) {
							$debug = unserialize(gzuncompress(base64_decode($_REQUEST['debug'])));
							echo "<p>REQUEST:</p>";
							echo "<pre>".htmlspecialchars(prettyXML($debug['request']))."</pre>";
							echo "<p>RESPONSE:</p>";
							echo "<pre>".htmlspecialchars(prettyXML($debug['response']))."</pre>";
						}
						*/
						echo '</td></tr></table>';
				...
				<form action="print_intraship_label.php" id="intraship_form">
					<table class="boxCenter" valign="top" style="background: #FFCC00">
						<?php
						...
						<tr class="main">
							<td colspan="4" align="right">
								<?php
									//Link zur Intraship Webseite
									//echo '<a target="_blank" class="button" href="'.xtc_href_link('print_intraship_label.php', 'oID='.$oID.'&action=intraship').'">'.$gmintraship->get_text('BUTTON_DHL').'</a>';
								?>
							</td>
						</tr>
						<tr class="main">
				...
				<p style="font: 0.8em sans-serif; text-align: right; color: #888;"><?php echo $gmintraship->module_version ?></p>
			</td>
		</tr>
	</table>
</body>
