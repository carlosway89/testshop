...
#   --------------------------------------------------------------
#   ButtonSendTellAFriendHandler.js 2014-01-07 gambio
#   Gambio GmbH
#   http://www.gambio.de
...
function ButtonSendTellAFriendHandler()
{
	...
	this.send_form = function()
	{
		...
		$('.tell_a_friend_fields').each(function(){inputs.push(this.name + '=' + encodeURIComponent(this.value)); });
		inputs.push('vvcode=' + encodeURIComponent($('#vvcode').val()));
		inputs.push('recaptcha_response_field=' + encodeURIComponent($('#recaptcha_response_field').val()));
		inputs.push('recaptcha_challenge_field=' + encodeURIComponent($('#recaptcha_challenge_field').val()));
