<?php
/* --------------------------------------------------------------
   new_attributes_include.php 2015-09-10 gm
   Gambio GmbH
   http://www.gambio.de
...
<tr>
	...
	if ($matches) {
		while ($line = xtc_db_fetch_array($result)) {
			...
			echo '<table class="gx-compatibility-table"' . ($totalOptionCount <= 120 ? ' data-gx-widget="checkbox"' : '') . '>';
			...
			if ($matches2) {
				...
				echo '<tr>' .
						'<td class="main gx-container" colspan="8">' .
							'<div class="text-center add-padding-10">No values under this option.</div>' .
				            '<br>' .
						'</td>' .
					'</tr>';
			}
