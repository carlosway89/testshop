	...
	function (data) {
		...
		fixes.push(function () {
			...
			if ($('div.gx-configuration-box').length) {
				$('.boxCenterWrapper').wrap('<div class="boxCenterAndConfigurationWrapper"></div>');
		...
		fixes.push(function () {
			var urlHelper = window.gx.libs.url_arguments;
		...
		fixes.push(function () {
			...
			var file = urlHelper.getCurrentFile(),
				doParameter = urlHelper.getParameterArray().do || '',
				largePages = [
					'gm_emails.php'
				],
				smallPages = [
		...
		fixes.push(function () {
			var urlHelper = window.gx.libs.url_arguments,
				...
				controllerAction = urlHelper.getParameterArray().do,
				largePages = [],
