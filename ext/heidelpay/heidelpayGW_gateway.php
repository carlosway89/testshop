...
*/
	...
	if((strpos($_SESSION['payment'], 'hp') === false) || (strpos($_SESSION['payment'], 'hp') != 0)){
		...
		if(in_array($activePayment, $bookingMode)){
			...
			if($booking == 3 || $booking == 4){
				...
				if(!empty($regData) && ($regData['uid'] != '') && ((($regData['expMonth'] == '0') && ($regData['expYear'] == '0')) || ($last > time())) && (($regData['shippingHash'] == $shippingHash) || ($getConf['shippinghash'] == 1))){
	...
	$tplVar['tplPath']			= 'templates/EyeCandy';
