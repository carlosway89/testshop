<?php
/* --------------------------------------------------------------
  gm_trusted_shop_id.php 2015-09-28 gm
  Gambio GmbH
  http://www.gambio.de
...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	if(isset($_POST['seal_cfg']))
	{
		...
		$service->videobox_enabled = isset($_POST['enable_videobox']) ? '1' : '0';

		if(isset($_POST['enable_videobox'])) {
			// make sure video box is enabled
			$boxes_id = false;
			$boxes_id_result = xtc_db_query("SELECT boxes_id FROM gm_boxes WHERE box_name = 'gm_trusted_shops_video' AND template_name = 'EyeCandy'");
			while($boxes_id_row = xtc_db_fetch_array($boxes_id_result)) {
				$boxes_id = $boxes_id_row['boxes_id'];
			}
			if($boxes_id == false) {
				// get position of widget box
				$widget_box_result = xtc_db_query("SELECT * FROM gm_boxes WHERE box_name = 'gm_trusted_shops_widget' AND template_name = 'EyeCandy'");
				$widget_box_pos = false;
				while($widget_box_row = xtc_db_fetch_array($widget_box_result)) {
					$widget_box_pos = str_replace('gm_box_pos_', '', $widget_box_row['position']);
				}
				// insert new entry for video box
				xtc_db_query("INSERT INTO gm_boxes SET template_name = 'EyeCandy', box_name = 'gm_trusted_shops_video', position = 'gm_box_pos_".($widget_box_pos+1)."', box_status = 1");
				$boxes_id = xtc_db_insert_id();
			}
			// make sure video box is active
			xtc_db_query("UPDATE gm_boxes SET box_status = 1 WHERE box_name = 'gm_trusted_shops_video'");
		}
	}
...
$videobox_enabled = $service->videobox_enabled;
		...
		<table border="0" width="100%" cellspacing="2" cellpadding="2">
			<tr>
				...
				<td class="boxCenter" width="100%" valign="top">
					<div class="pageHeading" style="background-image:url(images/gm_icons/gambio.png)">##HEADING_TITLE</div>
						...
						<?php if(!empty($certs)): ?>
							<div class="tscfg tsintegration">
								...
								<form action="" method="post" class="padded">
									...
									<?php /*
									<input type="checkbox" name="enable_seal" value="1" id="enable_seal" <?php echo $seal_enabled ? 'checked="checked"' : '' ?>>
									<label for="enable_seal">##TS_ENABLE_SEAL</label>
									<br>
									*/ ?>
									<input type="checkbox" name="enable_videobox" value="1" id="enable_videobox" <?php echo $videobox_enabled ? 'checked="checked"' : '' ?>>
									<label for="enable_videobox">##TS_ENABLE_VIDEOBOX</label>
									<br>
