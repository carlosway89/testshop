<?php
/* --------------------------------------------------------------
	PayoneCheckoutSuccessExtender.inc.php 2014-07-30 mabr
	Gambio GmbH
	http://www.gambio.de
		...
		if($order->info['payment_method'] == 'payone_elv' && empty($_SESSION['payone_elv_sepa_mandate_id']) == false)
		{
			...
			if($show_pdf_link)
			{
				...
				if($mandate_file !== false)
				{
					...
					$block .= ' <a target="_blank" href="'.GM_HTTP_SERVER.DIR_WS_DOWNLOAD_PUBLIC.$mandate_file.'">'.$payone->get_text('mandate_pdf').'</a>';
					$block .= '</div>';
