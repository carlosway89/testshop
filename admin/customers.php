	...
	switch ($_GET['action']) {
		...
		case 'update' :
			if($_SESSION['coo_page_token']->is_valid($_POST['page_token']))
			{
				...
				if (strlen_wrapper($customers_lastname) < ENTRY_LAST_NAME_MIN_LENGTH) {
					$error = true;
				...
				if ($error == false) {
					...
					$customer = $customerReadService->getCustomerById(MainFactory::create('Id', $customers_id));
					...
					$country = $countryService->getCountryById(MainFactory::create('Id', $entry_country_id));
					...
					if(ACCOUNT_STATE == 'true')
					{
						if($entry_zone_id > 0)
						{
							$countryZone = $countryService->getCountryZoneById(MainFactory::create('Id', $entry_zone_id));
						}
						else
						{
		...
		case 'deleteconfirm':
			if($_SESSION['coo_page_token']->is_valid($_POST['page_token']))
			{
				...
				/** @var CustomerWriteService $customerWriteService */
				$customerWriteService = StaticGXCoreLoader::getService('CustomerWrite');
				$customerWriteService->deleteCustomerById(MainFactory::create('Id', $customers_id));
...
<body
    ...
    data-button_dropdown-config_keys="customerOverviewDropdownBtn"
>
<!-- header //-->
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="0" class="gx-compatibility-table gx-customer-overview" data-gx-compatibility="customers/customers_table_controller">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent" style="min-width: 280px" align="right">&nbsp;</td>
              </tr>
