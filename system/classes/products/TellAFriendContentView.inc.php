<?php
/* --------------------------------------------------------------
   TellAFriendContentView.inc.php 2014-11-04 tw@gambio
   Gambio GmbH
   http://www.gambio.de
...
class TellAFriendContentView extends ContentView
{
	...
	protected function _validateInput($product)
	{
		...
		if (!$captchaIsValid || empty($this->email))
		{
			if(empty($this->email))
			{
	...
	protected function _prepareMail($email, $sender, $productLink)
	{
		...
		$text_html = htmlentities_wrapper($sender) . htmlentities_wrapper(GM_TELL_A_FRIEND_RECOMMENDS_1) . '"' . htmlentities_wrapper(STORE_NAME) . '"' . htmlentities_wrapper(GM_TELL_A_FRIEND_RECOMMENDS_2)
					 . '<br /><a href="' . $productLink . '" target="_blank">' . htmlentities_wrapper($productLink) . '</a>'
					 . '<br /><br />' . htmlentities_wrapper(GM_TELL_A_FRIEND_MESSAGE) . '<br />' . htmlentities_wrapper($message);
		
		xtc_php_mail($email, $sender, $this->email, $this->email, '', $email, $sender, '', '', GM_TELL_A_FRIEND_SUBJECT_1 . '"' . STORE_NAME . '"' . GM_TELL_A_FRIEND_SUBJECT_2, $text_html, html_entity_decode_wrapper($text));
