<?php
/* --------------------------------------------------------------
   manufacturers.php 2015-10-21 gm
   Gambio GmbH
   http://www.gambio.de
	...
	<div class="hidden">
		...
		switch ($_GET['action']) {
			...
			case 'edit':
				...
				$contents[] = array('text' => '<span class="options-title">' . TEXT_MANUFACTURERS_IMAGE . '</span>' . xtc_draw_file_field('manufacturers_image', false, 10) . '<br />' . $mInfo->manufacturers_image);
			...
			default:
				if (is_object($mInfo)) {
					...
					$contents[] = array('text' => '<br />' . xtc_info_image($mInfo->manufacturers_image, htmlspecialchars($mInfo->manufacturers_name, ENT_QUOTES)));
