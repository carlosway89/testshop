...
class GMGMotion_ORIGIN
{
	...
	function get_form_data($p_image_nr = 0)
	{
		$c_image_nr = (int)$p_image_nr;
		...
		{
			$t_data_array = $this->load($_GET['pID']);
			...
			if($c_image_nr == 0)
			{
				...
				if(xtc_db_num_rows($t_get_show_image) == 1)
				{
					...
					$t_products_image = $t_show_image['products_image'];
				}
			}
			else
			{
				...
				if(xtc_db_num_rows($t_get_show_image) == 1)
				{
					...
					$t_products_image = $t_show_image['image_name'];
				}
			}
			
			if(isset($t_data_array[$p_image_nr]))
			{
				$t_gmotion_image = ' checked="checked"';
				
				$t_position_from = $t_data_array[$p_image_nr]['POSITION_FROM'];
				$t_position_to = $t_data_array[$p_image_nr]['POSITION_TO'];
				$t_zoom_from = $t_data_array[$p_image_nr]['ZOOM_FROM'];
				$t_zoom_to = $t_data_array[$p_image_nr]['ZOOM_TO'];
				$t_duration = $t_data_array[$p_image_nr]['DURATION'];
				$t_sort_order = $t_data_array[$p_image_nr]['SORT_ORDER'];
			}
			else
			{
				$t_gmotion_image = '';
	...
	function save()
	{
		if(isset($_POST) && isset($_GET))
		{
			...
			if($f_post_array['gm_gmotion_activate'] == '1')
			{
				...
				{
					// delete
					if(isset($f_get_array['pID']) && (empty($f_post_array['gm_gmotion_image_'.$i]) 
							|| (is_array($f_post_array['del_mo_pic']) && in_array($f_post_array['products_previous_image_'.$i], $f_post_array['del_mo_pic'])))
							|| ($i == 0 && isset($f_post_array['del_pic'])))
					{
						if($i == 0 && isset($f_post_array['del_pic']))
						{
					...
					{
						$t_check = xtc_db_query("SELECT gm_gmotion_id 
													FROM " . GM_TABLE_GM_GMOTION . "
													WHERE 
														products_id = '" . $c_products_id . "'
														AND image_nr = '" . $i . "'");
						
						if((int)$f_post_array['gm_gmotion_duration_'.$i] > 0)
						{
							$c_gm_gmotion_duration = (int)$f_post_array['gm_gmotion_duration_'.$i];
						}
						else
						{
						...
						if(xtc_db_num_rows($t_check) == 1)
						{
							$t_update = xtc_db_query("UPDATE " . GM_TABLE_GM_GMOTION . "
														SET
															position_from = '" . gm_prepare_string($f_post_array['gm_gmotion_position_from_'.$i]) . "',
															position_to = '" . gm_prepare_string($f_post_array['gm_gmotion_position_to_'.$i]) . "',
															zoom_from = '" . (double)$f_post_array['gm_gmotion_zoom_from_'.$i] . "',
															zoom_to = '" . (double)$f_post_array['gm_gmotion_zoom_to_'.$i] . "',
															duration = '" . $c_gm_gmotion_duration . "',
															sort_order = '" . (int)$f_post_array['gm_gmotion_sort_order_'.$i] . "'
														WHERE
															products_id = '" . $c_products_id . "'
															AND image_nr = '" . $i . "'
															");
						}						
						elseif(!empty($f_post_array['products_previous_image_'.$i]) || (isset($_FILES['mo_pics_'.($i-1)]) && $_FILES['mo_pics_'.($i-1)]['error'] === 0) || ($i == 0 && $_FILES['products_image']['error'] === 0) )
						{
