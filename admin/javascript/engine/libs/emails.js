 ...
 * ## Emails Library
 *
 * This library contains all the admin/emails page common functionality and is used
 * by the page controllers. You might also use this library in other pages where you
 * need to trigger specific email operations in the server.
 *
 * You will need to provide the full URL in order to load this library as a dependency to a module:
 * e.g. jse.core.config.get('shopUrl') + '/admin/html/assets/javascript/engine/libs/emails'
 *
 * Required Translation Sections: 'admin_labels', 'buttons', 'db_backup', 'emails', 'lightbox_buttons', 'messages'
 *
 * @namespace Admin/Libs/emails
 */
(function (/** @lends Admin/Libs/emails */ exports) {
	...
	/**
	 * Get email information from modal (DOM).
	 *
	...
	/**
	 * Colorize modal buttons for Edit Mode
	 *
	...
	/**
	 * Colorize modal buttons for Preview Mode
	 *
	 * jQuery UI does not support direct addition of classes to the dialog buttons,
	 * so we need to apply the classes during the "create" event of the dialog.
	 *
	...
	/**
	 * Delete old attachments from selected removal date and before.
	 *
	...
	/**
	 * Get attachments size in MB and refresh the UI.
	 *
	 ...
	 * attachments.
	 *
	 * @param $target jQuery selector for the element to contain the size info.
	 * @param ajaxUrl (optional) Specific ajaxUrl to be used for the request.
	 *
	...
	/**
	 * Update modal tabs counters.
	 *
	 * Display item number on tabs so that users know how many items there are
	...
	/**
	 * Get an object array with the selected emails of the main emails table.
	 *
