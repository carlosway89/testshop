...
if($_GET['action'] == "address_edit")
{
	...
	$sql_data_array = array('customers_vat_id' => xtc_db_prepare_input($_POST['customers_vat_id']),
							...
							'delivery_gender' => xtc_db_prepare_input($_POST['delivery_gender']),
							...
							'delivery_country_iso_code_2' => xtc_db_prepare_input($_POST['delivery_country_iso_code_2']),
							...
							'billing_gender' => xtc_db_prepare_input($_POST['billing_gender']),
							...
							'billing_country_iso_code_2' => xtc_db_prepare_input($_POST['billing_country_iso_code_2']),
...
if($_GET['action'] == "product_ins")
{
	...
	$sql_data_array = array('orders_id' => (int)$_POST['oID'],
							...
							'products_price' => (double)$price,
							'products_discount_made' => '',
							'final_price' => (double)$final_price,
							'products_tax' => (double)$tax_rate,
							'products_quantity' => (double)$_POST['products_quantity'],
							...
							'products_shipping_time' => xtc_db_prepare_input($t_shipping_time['shipping_status_name'])
	);
...
if($_GET['action'] == "product_option_ins")
{
	...
	$sql_data_array = array('orders_id' => (int)$_POST['oID'],
							...
							'options_values_price' => (double)$products_attributes['options_values_price']);

	$insert_sql_data = array('price_prefix' => xtc_db_prepare_input($products_attributes['price_prefix']),
								'options_id' => (int)$products_attributes['options_id'],
								'options_values_id' => (int)$products_attributes['options_values_id']);
...
{
	$sql_data_array = array('payment_method' => xtc_db_prepare_input($_POST['payment']),
...
if($_GET['action'] == "shipping_edit")
{
	if(isset($_POST['shipping']) && empty($_POST['shipping']) == false)
	{
		if($_POST['shipping'] != 'no_shipping')
		{
			...
			$coo_lang_file_master->init_from_lang_file('lang/' . $order->info['language'] . '/modules/shipping/' . $module);
			if(defined('MODULE_SHIPPING_' . strtoupper($_POST['shipping']) . '_TEXT_TITLE'))
			{
				...
				$sql_data_array = array('shipping_method' => xtc_db_prepare_input($shipping_text),
