 ...
 *
 * Adds dropdown functionality to a button. You are able to add manually through HTML or
 * dynamically through JS more <li> items to the list and apply custom event handlers for
 * them.
 *
 * #### Example:
 * ```html
 *
 * <!-- This represents the containing element -->
 * <div
 *      data-gx-widget="button_dropdown"
 *      data-button_dropdown-config_keys="order-single order-multi"
 *      data-button_dropdown-user_id="2"
 * >
 *      <!-- This element respresent a dropdown widget -->
 *      <div
 *          data-use-button_dropdown="true" <!-- Important! -->
 ...
 * ```
 *
 * The containing div element initializes the widget, retrieves the elements and fetches
 * for each user configuration key specified `config_keys` the respective values.
 * `user_id` is required to obtain the right configuration value for the current logged in user.
 *
 * The real dropdown button elements are identified through `use-button_dropdown`.
 * If you assign a key to `config_key` make sure it is
 * registered in the containing `config_keys` attribute, so the value can be set after it has been fetched from server.
 * Optionally passing custom class thorugh `custom_caret_btn_class` is possible to stylize the caret button.
 *
