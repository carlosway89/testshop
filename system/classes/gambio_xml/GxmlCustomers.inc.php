<?php
/* --------------------------------------------------------------
   GxmlCustomers.inc.phtml 2015-06-19 gm
   Gambio GmbH
   http://www.gambio.de
...
class GxmlCustomers extends GxmlMaster
{
		...
		try
		{
			...
			foreach($request->parameters->customers->customer as $node)
			{
				...
				//{
					...
					$customer->getDefaultAddress()->setCustomerId(MainFactory::create('Id', $customer->getId()));
	...
	{
		$customer = $this->customerService->getCustomerById(MainFactory::create('Id', (int)$node->customer_id));
		$this->customerService->deleteCustomerById(MainFactory::create('Id', $customer->getId()));
	...
	protected function _createCustomerByNode(SimpleXMLElement $node)
	{
		...
		{
			$customer->setId(MainFactory::create('Id', (int)$node->customer_id));
			$address->setCustomerId(MainFactory::create('Id', (int)$node->customer_id));
		}
	...
	protected function _createAddressByNode(SimpleXMLElement $node)
	{
		...
		{
			$address->setId(MainFactory::create('Id', (int)$node->address_id));
		}
		...
		$country        = $countryService->getCountryById(MainFactory::create('Id', (int)$node->country_id));
		$address->setCountry($country);

		// Fetch zone through CountryService
		$zone = $countryService->getCountryZoneById(MainFactory::create('Id', (int)$node->zone_id));
