<?php
/* --------------------------------------------------------------
  ShoppingCartDropdownBoxContentView.inc.php 2014-11-08 gm
  Gambio GmbH
  http://www.gambio.de
	...
	protected function add_products()
	{
		...
		for($i = 0, $n = sizeof($t_products); $i < $n; $i ++)
		{
			...
			$this->content_array['products'][] = array(
				...
				'LINK' => xtc_href_link(FILENAME_PRODUCT_INFO, xtc_product_link($t_products[$i]['id'], $t_products[$i]['name'])),
				'NAME' => $t_products[$i]['name'],
				'IMAGE' => $t_image,
				'PRICE' => (strlen(trim($t_price)) > 0 ? $t_price : $xtPrice->xtcFormat($t_products[$i]['price'], true)),
