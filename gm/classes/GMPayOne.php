...
class GMPayOne_ORIGIN {
	...
	public function getPaymentTypes() {
		...
		$payment_types = array(
			...
			'installment' => array('billsafe', 'commerzfinanz'),
		);
	...
	public function adjustSortOrders() {
		...
		foreach($modules as $pmodule)
		{
			...
			if(file_exists($pmodule_fullpath))
			{
				...
				$modules_sorttmp[] = sprintf('%02d##%s', $pmodule_sort_order, basename($pmodule));
			}
			else
			{
				continue;
