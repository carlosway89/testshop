...
class ShopgateItemModel extends Shopgate_Model_Catalog_Product
{
        ...
        if (($this->config->getVariationType() == SHOPGATE_SETTING_VARIATION_TYPE_ATTRIBUTE
            ...
            $qry
                ...
                WHERE pa.products_id = '" . $product['products_id'] . "'
                    ...
                    . mysql_real_escape_string($optionsAsTextfieldIds) . "))" : ")")
        ...
        if (($this->config->getVariationType() == SHOPGATE_SETTING_VARIATION_TYPE_PROPERTY
            ...
            $qry
                ...
                "`ppc`.`vpe_value` as `products_vpe_value`,
                    ...
                    `pvd`.`values_name`," .
                (($this->ggxVersion['main_version'] >= 2 && $this->ggxVersion['sub_version'] >= 1) ? ""
                ...
                . ((!empty($optionsAsTextfieldIds)) ? " AND pa.options_id NOT IN (" . mysql_real_escape_string(
    ...
    public function getInputFields($item, $inputFieldsFromGxCustomizer, $asArray = false)
    {
        ...
        $qry
            ...
            . ((!empty($optionsAsTextfieldIds)) ? " OR pa.options_id IN (" . mysql_real_escape_string(
                    $optionsAsTextfieldIds
                ) . "))" : ")")
    ...
    public function generateDeepLink($productsId, $productsName, $gmSEOBoostProductsEnabled, $gmSEOBoost)
    {
        ...
        $deepLink = $httpServer . DIR_WS_CATALOG . $deepLink;
    ...
    {
        $category_numbers = $this->getProductCategoryNumbers($item['products_id']);
        $numbers = array();
        foreach ($category_numbers as &$category) {
     ...
     * @return array
     */
    private function getProductCategoryNumbers($itemNumber)
    {
    ...
    public function getProductDescription($item, $desc, $shortDesc, $exportProductsManagedFiles, $exportDescriptionType)
    {
        ...
        if ($exportProductsManagedFiles) {
            ...
            while ($row = xtc_db_fetch_array($queryResult)) {
                ...
                $iconImageUrl = rtrim(HTTP_SERVER . DIR_WS_CATALOG, '/') . "/admin/images/icons/icon_{$fileExt}.gif";
    ...
    public function generateProductQuery($limit, $offset, $uids)
    {
        ...
        $qry .= (!empty($optionsAsTextfieldIds))
            ? " AND pa.options_id NOT IN (" . mysql_real_escape_string($optionsAsTextfieldIds) . ")"
            : "";
    ...
    {
        // according to a blog post on the gambio blog, the attribute-vpe and -ean is supportet since GambioGX 1.0.14
    ...
    public function calculateWeight($product)
    {
        ...
        if (!empty($product["products_weight"])) {
            $weight = $product["products_weight"] * 1000;
        }
    ...
    public function generatePriceStatus($item, &$oldPrice)
    {
        ...
        switch ($item['gm_price_status']) {
            case 2: // "not available for purchase":
                ...
                $gmPriceStatusItemFields['basic_price']    = SHOPGATE_PLUGIN_PRICE_STATUS_TEXT_NOT_AVAILABLE;
                break;
            case 1: // "price on request"
                if ($this->config->getExportPriceOnRequest() == SHOPGATE_SETTING_EXPORT_PRICE_ON_REQUEST_WITHOUT_PRICE
                    ...
                    $gmPriceStatusItemFields['basic_price']     = SHOPGATE_PLUGIN_PRICE_ON_REQUEST_BASIC_PRICE_TEXT;
                    $gmPriceStatusItemFields['available_text']  = SHOPGATE_PLUGIN_PRICE_STATUS_TEXT_NOT_AVAILABLE;
    ...
    public function getProductCodes($uid)
    {
        ...
        if ($databaseHelper->checkTable("products_item_codes")) {
            ...
            $query = "SELECT code_isbn AS `isbn`, code_upc AS `upc`, code_mpn AS `mpn`, code_jan AS `jan` FROM " .
