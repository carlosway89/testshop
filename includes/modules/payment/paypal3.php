...
class paypal3_ORIGIN {
	...
	public function __construct()
	{
		...
		$config_button = '<br><br><a href="'.xtc_href_link('admin.php', 'do=PayPalConfiguration').'" class="button" style="margin: auto; background-color: #E30000;">'.$this->text->get_text('configure').'</a><br>';
		$this->description = @constant('MODULE_PAYMENT_'.strtoupper($this->code).'_TEXT_DESCRIPTION') . ($this->enabled ? $config_button : '');
	...
	public function _selectionPayPalPlus()
	{
		try {
			...
			$ppplusSnippet .= '<script> var ppp; var initPPP = function(thirdPartyPayments) { ppp = PAYPAL.apps.PPP('.$ppplusConfigJSON.'); };</script>';
	...
	protected function _hasLowestPriority()
	{
		$minSortOrderQuery =
			"SELECT
				MIN(configuration_value) AS MIN_SORT_ORDER
			FROM
				`configuration`
			WHERE
		...
		{
			$minSortOrder = (double)$row['MIN_SORT_ORDER'];
		}
	...
	public function selection()
	{
		...
		*/
		...
		if($stateRequired)
		{
			$deliveryCountry = $countryService->getCountryById(new Id($GLOBALS['order']->delivery['country']['id']));
			if($countryService->countryHasCountryZones($deliveryCountry))
			{
				$countryZones = $countryService->findCountryZonesByCountryId(new Id($GLOBALS['order']->delivery['country']['id']));
	...
	function pre_confirmation_check()
	{
		...
		{
			$countryService = StaticGXCoreLoader::getService('Country');
			$paypalZone = $countryService->getCountryZoneById(new Id((int)$_POST['paypal_zone']));
		...
		if(!isset($_GET['paymentId']))
		{
			...
			elseif($_SESSION['paypal_payment']['type'] == 'plus')
			{
				...
				try {
					...
					{
						$this->logger->notice('Updating PLUS payment with shipping/billing address and invnum: '.$_SESSION['paypal_payment']['id']);
