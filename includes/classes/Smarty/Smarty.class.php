...
class Smarty3 extends Smarty_Internal_TemplateBase
{
	...
	{
		// try absolute path
		$usermodFilePath = get_usermod($p_template);
		if($usermodFilePath != $p_template) return $usermodFilePath;

		if((defined('DIR_FS_CATALOG') && strpos($p_template, DIR_FS_CATALOG) !== 0) || defined('DIR_FS_CATALOG') == false)
		{
			// try relative path
			$templateDir = $this->template_dir . '/';
			if(strpos($p_template, $templateDir) === 0)
			{
				$templateDir = '';
			}

			$usermodFilePath = get_usermod($templateDir . $p_template);
			if($usermodFilePath != $templateDir . $p_template) return $usermodFilePath;
		}
