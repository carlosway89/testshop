<?php
/* --------------------------------------------------------------
   DBBackupAjaxHandler.inc.php 2014-02-06 tb@gambio
   Gambio GmbH
   http://www.gambio.de
		...
		switch( $t_action )
		{
			...
			case 'bundle_db_backup':
				...
				switch( $c_bundle_type )
				{
					...
					default:
						print_r($_GET);
						trigger_error( 'bundle_type not found: '. htmlentities_wrapper( $c_bundle_type ) );
						return false;
				}
			...
			default:
				print_r($_GET);
				trigger_error( 't_action_request not found: '. htmlentities_wrapper( $t_action ) );
				return false;
		}
