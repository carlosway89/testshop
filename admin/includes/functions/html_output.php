 	...
 	if(is_array($values))
   	{
         foreach ($values as $key=>$val) {
             ...
             if ( ((strlen($val['id']) > 0) && ($GLOBALS[$name] == $val['id'])) || ($default != '' && $default == $val['id']) ) {
               $field .= ' SELECTED';    
             }    
  ...
  function xtc_sorting($page,$sort) {
      ...
      switch ($page) {
          case FILENAME_CUSTOMERS:
			  //GM_MOD:
			  $nav='<a class="' . $sort . '" . href="'.xtc_href_link(FILENAME_CUSTOMERS,'sorting='.$sort.'&'.xtc_get_all_get_params(array('action','sorting'))).'">';
			  $nav.=xtc_image(DIR_WS_ICONS . 'sort_down.gif', '', '14' ,'12', 'class="sort_image"').'</a>';
			  $nav.='<a class="' . $sort . '-desc " . href="'.xtc_href_link(FILENAME_CUSTOMERS,'sorting='.$sort.'-desc&'.xtc_get_all_get_params(array('action','sorting'))).'">';
          ...
          case FILENAME_CATEGORIES:
			  //GM_MOD:
			  $nav='<div><a class="' . $sort . '" href="'.xtc_href_link(FILENAME_CATEGORIES,'sorting='.$sort.'&'.xtc_get_all_get_params(array('action','sorting'))).'">';
			  $nav.=xtc_image(DIR_WS_ICONS . 'sort_down.gif', '', '14' ,'12', 'class="sort_image"').'</a>';
			  $nav.='<a class="' . $sort . '-desc " . href="'.xtc_href_link(FILENAME_CATEGORIES,'sorting='.$sort.'-desc&'.xtc_get_all_get_params(array('action','sorting'))).'">';
			  $nav.= xtc_image(DIR_WS_ICONS . 'sort_up.gif', '', '14' ,'12', 'class="sort_image"').'</a></div>';

			  break;

          default:
			  $nav='<div><a class="' . $sort . '" href="'.xtc_href_link($page,'sorting='.$sort.'&'.xtc_get_all_get_params(array('action','sorting'))).'">';
			  $nav.=xtc_image(DIR_WS_ICONS . 'sort_down.gif', '', '14' ,'12', 'class="sort_image"').'</a>';
			  $nav.='<a class="' . $sort . '-desc " . href="'.xtc_href_link($page,'sorting='.$sort.'-desc&'.xtc_get_all_get_params(array('action','sorting'))).'">';
			  $nav.= xtc_image(DIR_WS_ICONS . 'sort_up.gif', '', '14' ,'12', 'class="sort_image"').'</a></div>';
