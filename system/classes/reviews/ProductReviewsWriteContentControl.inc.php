<?php
/* --------------------------------------------------------------
   ProductReviewsWriteContentControl.inc.php 2015-10-30 gm
   Gambio GmbH
   http://www.gambio.de
...
class ProductReviewsWriteContentControl extends DataProcessing
{
	...
	public function proceed($p_coo_product, $p_customer_id, $p_languages_id = null)
	{
		if($p_languages_id === null)
		{
				...
				if ($this->gmSEOBoost->boost_products)
				{
					$productLink = xtc_href_link(
						$this->gmSEOBoost->get_boosted_product_url(
							$p_coo_product->data['products_id'],
							$p_coo_product->data['products_name']
						)
					);
				}
				else
