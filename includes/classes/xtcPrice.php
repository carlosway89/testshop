...
class xtcPrice_ORIGIN
{
	...
	public function xtcGetPrice($p_products_id, $p_format_price, $p_quantity, $p_tax_class_id, $p_products_price, $p_return_array = 0, $p_customers_id = 0, $p_include_special = true, $p_consider_properties = false, $p_combis_id = 0)
	{
		...
		{
			return $this->xtcShowNote($p_return_array);
		}
		...
		if($this->gm_check_price_status($p_products_id) != 0)
		{
			if($this->gm_check_price_status($p_products_id) == 2)
			{
				...
				if($t_price == 0)
				{
					return $this->gm_show_price_status($this->gm_check_price_status($p_products_id), $p_return_array);
				}
			}
			else
			{
		...
		if(empty($t_discount) === false)
		{
			...
			return $this->xtcFormatSpecialDiscount($p_products_id, $t_discount, $t_products_price, $p_format_price, $p_return_array, 0, $t_combi_price);
		}
		return $this->xtcFormat($t_products_price, $p_format_price, 0, false, $p_return_array, $p_products_id);
	}
	...
	public function xtcFormatSpecialGraduated($p_products_id, $p_price, $p_old_price, $p_format, $p_return_array = 0)
	{
		...
		if($p_old_price == 0)
		{
			return $this->xtcFormat($p_price, $p_format, 0, false, $p_return_array);
		}
		...
		{
			return $this->xtcFormatSpecialDiscount($p_products_id, $t_discount, $p_old_price, $p_format, $p_return_array);
		}
