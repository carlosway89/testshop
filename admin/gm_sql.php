...
if(isset($_POST['query']))
{
	if($_SESSION['coo_page_token']->is_valid($_POST['page_token']))
	{
		...
		for($i = 0; $i < count($gm_query); $i++)
		{
			...
			if(!$gm_query_result) {
				$gm_query_result_output .= GM_SQL_ERROR . mysql_error() . '<br />';
			}
		}
				...
				<div id="gm_sql_output" style="overflow:auto; height:500px;">
					<table class="gx-modules-table gx-container" border="0" style="border-collapse: collapse">
						...
						for($i = 0; $i < mysql_num_rows($gm_query_result); $i++){
							echo '<tr id="result_row_'.$i.'" class="dataTableRow" onclick="set_color('.$i.')">';
							for($h = 0; $h < mysql_num_fields($gm_query_result); $h++){
								$name = mysql_field_name($gm_query_result, $h);
								$value = htmlentities(mysql_result($gm_query_result, $i, $name));
