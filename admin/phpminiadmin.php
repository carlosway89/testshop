...
<br/>";
   if ($is_sht) $sqldr.="&nbsp;Database:&nbsp;&#183;<a href='$url&page_token=" . $t_page_token . "&q=show+table+status'>Show status</a>";
   $sqldr.="</div>";
 }
 if ($is_sht){
 ...
 while($row=mysql_fetch_row($sth)){
   ...
   for($i=0;$i<$fields_num;$i++){
      ...
      if ($is_sht && $i==0 && $v){
         ...
         $url='?'.$xurl."&db=$v";
         $v="<a href=\"$url&page_token=" . $t_page_token . "&q=show+tables\">$v</a></td>"
         ."<td><a href=\"$url&page_token=" . $t_page_token . "&q=show+create+database+`$v`\">sct</a></td>"
...
<?php if ($_SESSION['is_logged'] && $dbh){ ?>
 | 
<a href="?<?php echo $xurl?>&page_token=<?php echo $t_page_token; ?>&q=show+databases">Databases</a>: <select name="db" onChange="frefresh()"><option value='*'> - select/refresh -</option><option value=''> - show all -</option><?php echo get_db_select($dbn)?></select>
<?php if($dbn){ $z=" &#183;<a href='$self?$xurl&db=$dbn"; ?>
<?php echo $z?>&page_token=<?php echo $t_page_token; ?>&q=show+tables'>show tables</a>
<?php echo $z?>&page_token=<?php echo $t_page_token; ?>&q=show+table+status'>status</a>
...
<input type="submit" value=" Apply "><input type="button" value=" Cancel " onclick="window.location='<?php echo $self?>?XSS=<?php echo $_SESSION['XSS']; ?>&page_token=<?php echo $t_page_token; ?>'">
</div>
</center>
<?php
 print_footer();
...
<input type="hidden" name="page_token" value="<?php echo $t_page_token; ?>">
<input type="submit" value=" Download "><input type="button" value=" Cancel " onclick="window.location='<?php echo $self.'?'.$xurl.'&db='.$DB['db']?>&page_token=<?php echo $t_page_token; ?>'">
</div>
</center>
<?php
 print_footer();
...
<input type="hidden" name="page_token" value="<?php echo $t_page_token; ?>">
<input type="submit" value=" Upload and Import " onclick="return ays()"><input type="button" value=" Cancel " onclick="window.location='<?php echo $self.'?'.$xurl.'&db='.$DB['db']?>&page_token=<?php echo $t_page_token; ?>'">
</div>
<br /><br /><br />
<!--
