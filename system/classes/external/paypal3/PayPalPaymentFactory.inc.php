<?php
/* --------------------------------------------------------------
	PayPalPaymentFactory.inc.php 2015-11-16
	Gambio GmbH
	http://www.gambio.de
...
class PayPalPaymentFactory
{
	...
	public function makePaymentArrayFromOrder(order $order, $mode = 'ecm', $state = null, $countryCode = null)
	{
		...
		if($shippingCost > 0)
		{
			$paymentArray['transactions'][0]['amount']['details']['shipping'] = number_format($shippingCost, 2, '.', '');
		}
		...
		if($remaining_fee > 0)
		{
			$paymentArray['transactions'][0]['amount']['details']['handling_fee'] = number_format($remaining_fee, 2, '.', '');
		}
		elseif($remaining_fee < 0)
		{
			$paymentArray['transactions'][0]['amount']['details']['shipping_discount'] = number_format($remaining_fee, 2, '.', '');
		}
		...
		if(!empty($order->delivery['lastname']) && $mode != 'plus')
		{
			$paymentArray['transactions'][0]['item_list']['shipping_address'] = array(
								...
								"country_code" => substr($order->delivery['country']['iso_code_2'], 0, 2),
							);
			if($state !== null)
			{
	...
	public function updatePaymentFromOrder($payment_id, order $order)
	{
		...
		$patchArray[] = array(
			...
			"value" => array(
				...
				//"country_code" => $order->delivery['country_iso_2']
			)
		);
		...
		$paymentArray = array(
				...
				"transactions" => array(
					array(
						"amount" => array(
							"currency" => $order->info['currency'],
							"total" => number_format((double)$amount, 2, '.', ''),
						),
