<?php
/* --------------------------------------------------------------
  LanguageTextManager.inc.php 2015-10-08
  Gambio GmbH
  http://www.gambio.de
...
class LanguageTextManager
{
	...
	protected function _initSection($p_section, $p_languageId = 0)
	{
		...
		if($this->useFallback && isset($this->languages[DEFAULT_LANGUAGE]['language_id'])
		   && $languageId !== $this->languages[DEFAULT_LANGUAGE]['language_id']
		)
		{
			// read section from fallback language
			$this->_readSectionFromDB($section, $this->languages[DEFAULT_LANGUAGE]['language_id']);
		}
	...
	protected function _readSectionFromDB($p_section, $p_languageId)
	{
		...
		$query = "	SELECT
					...
					FROM
						%s
					WHERE
						`section_name` = '%s' AND `language_id` = '%s';";

		$query = sprintf($query, self::$languagePhrasesCacheTable, xtc_db_input($p_section), (int)$p_languageId);
		...
		while(($row = xtc_db_fetch_array($result)))
		{
			...
			$sectionArray[$phraseName] = $phraseText;
		}
	...
	protected function _initLanguages()
	{
		...
		while($row = xtc_db_fetch_array($result))
		{
			$this->languages[$row['code']]                = array();
			$this->languages[$row['code']]['language_id'] = (int)$row['languages_id'];
			$this->languages[$row['code']]['directory']   = $row['directory'];
		}
	}
	...
	protected function _getSectionName($p_section)
	{
		...
		if(isset($this->sectionMappings[$p_section]))
		{
			$p_section = $this->sectionMappings[$p_section];
		}
