...
class AdminOrderOverviewTableExtenderComponent extends ExtenderComponent
{
	...
	/**
	 * Executes the standard procedure for this extender
	 *
	 * @param \IdInterface $orderId The order ID of the current row (can be used in overloads)
	 */
	 ...
	 */
	public function getRenderedContentsCells(IdInterface $orderId)
	{
		$html    = '';
		$columns = array_keys($this->v_output_buffer);
		sort($columns);
		foreach($columns as $columnName)
		{
			$html .= $this->renderContentCell($columnName, $orderId);
		}
	 ...
	 */
	protected function renderContentCell($columnName, IdInterface $orderId)
	{
	 ...
	 *
	 * @param $columnName The name of the column (just for identification)
	 * @param $classes    Custom classes (space separated)
	 */
