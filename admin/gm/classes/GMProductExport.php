...
class GMProductExport_ORIGIN {
	...
	function create_csv($p_prod_id_array = array(), $p_filename = '') {
		...
		while ($t_products = xtc_db_fetch_array($t_export_query)) {
			...
			$t_gm_get_shippingtime = mysql_query("SELECT shipping_status_name, number_of_days
					FROM shipping_status
					WHERE shipping_status_id = '" . $t_products['products_shippingtime'] . "'
					AND language_id = '" . $_SESSION['languages_id'] . "'");
			if (mysql_num_rows($t_gm_get_shippingtime) == 1) {
				$t_gm_shippingtime = mysql_fetch_array($t_gm_get_shippingtime);
			...
			$t_num_rows = mysql_num_rows($t_attributes_query);					
			// tax-free price
