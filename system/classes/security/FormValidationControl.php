...
class FormValidationControl extends BaseClass
{
	...
	public function validate_address(AddressModel $p_address, $p_block_packstation = false)
	{
		...
		{
			if($p_address->get_('entry_gender') != 'm' && $p_address->get_('entry_gender') != 'f')
			{
				$t_error_array['error_gender'] = ENTRY_GENDER_ERROR;
			}
		}

		if(strlen_wrapper($p_address->get_('entry_firstname')) < ENTRY_FIRST_NAME_MIN_LENGTH)
		{
			$t_error_array['error_first_name'] = sprintf(ENTRY_FIRST_NAME_ERROR, ENTRY_FIRST_NAME_MIN_LENGTH);
		}

		if(strlen_wrapper($p_address->get_('entry_lastname')) < ENTRY_LAST_NAME_MIN_LENGTH)
		{
		...
		{
			if(strlen_wrapper($p_address->get_('entry_company')) > 0 && strlen_wrapper($p_address->get_('entry_company')) < ENTRY_COMPANY_MIN_LENGTH)
			{
		...
		if($p_block_packstation === true && preg_match('/.*(packstation|postfiliale|filiale).*/i', $p_address->get_('entry_street_address')) == 1)
		{
		...
		if(ACCOUNT_STATE == 'true' && $p_address->get_('entry_country_id') > 0)
		{
			...
			if($t_entry_state_has_zone == false && strlen_wrapper($p_address->get_('entry_state')) < ENTRY_STATE_MIN_LENGTH)
			{
