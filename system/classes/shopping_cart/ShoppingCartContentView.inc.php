...
class ShoppingCartContentView extends ContentView
{
	...
	protected function _setShippingAndPaymentInfoLink()
	{
		...
		if($contentTitleArray = xtc_db_fetch_array($result))
		{
			...
			$contentUrl = xtc_href_link(FILENAME_POPUP_CONTENT, 'coID=' . (int)SHIPPING_INFOS . '&lightbox_mode=1' . $sefParameter, 'SSL');
	...
	protected function _setOrderDetailsCartContentView(&$p_products)
	{
		$orderDetailsCart = MainFactory::create_object('OrderDetailsCartContentView');

		$giftCart = MainFactory::create_object('GiftCartContentView');
		$orderDetailsCart->setGiftCartContentView($giftCart);

		$cartShippingCostsControl = MainFactory::create_object('CartShippingCostsControl', array(), true);
		$orderDetailsCart->setCartShippingCostsControl($cartShippingCostsControl);

		$cartShippingCostsContentView = MainFactory::create_object('CartShippingCostsContentView');
		$orderDetailsCart->setCartShippingCostsContentView($cartShippingCostsContentView);

		$gprintContentManager = MainFactory::create_object('GMGPrintContentManager');
		$orderDetailsCart->setGprintContentManager($gprintContentManager);

		//$t_view_html = $giftCart->get_html();

		$langFileMaster = MainFactory::create_object('LanguageTextManager', array(), true);
		$orderDetailsCart->setLangFileMaster($langFileMaster);

		$propertiesControl = MainFactory::create_object('PropertiesControl');
		$orderDetailsCart->setPropertiesControl($propertiesControl);

		$propertiesView = MainFactory::create_object('PropertiesView');
		$orderDetailsCart->setPropertiesView($propertiesView);
		$orderDetailsCart->setProducts($p_products);

		$main = new main();
		$orderDetailsCart->setMain($main);

		$orderDetailsCart->setXtcPrice($this->xtcPrice);
		
		$orderDetailsHtml = $orderDetailsCart->get_html();
