...
switch($_POST['action'])
{
	case 'test_db_connection':
		...
		if(!$db_error)
		{
			...
			{
				@mysql_query("DROP TABLE `gambio_test_db`");
				
				$t_create = @mysql_query("CREATE TABLE `gambio_test_db` (
							`id` INT NOT NULL ,
							PRIMARY KEY ( `id` )
							) ENGINE = MYISAM");
				if($t_create === false)
				...
				{
					$t_insert = @mysql_query("INSERT INTO `gambio_test_db` SET `id` = 1");
					if($t_insert === false)
					{
						$t_output = 'INSERT';
						break;
					...
					{
						$t_update = @mysql_query("UPDATE `gambio_test_db` SET `id` = 2");
						if($t_update === false)
						{
							$t_output = 'UPDATE';
							break;
						...
						{
							$t_select = @mysql_query("SELECT * FROM `gambio_test_db`");
							if($t_select === false)
							{
								$t_output = 'SELECT';
								break;
							...
							{
								$t_alter = @mysql_query("ALTER TABLE `gambio_test_db` CHANGE `id` `id_new` INT( 11 ) NOT NULL");
								if($t_alter === false)
								{
									$t_output = 'ALTER';
									break;
								...
								{
									$t_delete = @mysql_query("DELETE FROM `gambio_test_db` WHERE `id_new` = 2");
									if($t_delete === false)
									{
										$t_output = 'DELETE';
										break;
									...
									{
										$t_drop = @mysql_query("DROP TABLE `gambio_test_db`");
										if($t_drop === false)
										{
											$t_output = 'DROP';
											break;
