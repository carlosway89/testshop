...
class ShopgateLocationModel extends ShopgateObject
{
    ...
    public function getTaxClasses()
    {
        ...
        if (!$result) {
            throw new ShopgateLibraryException(
                ShopgateLibraryException::PLUGIN_DATABASE_ERROR, "no tax class found: " . mysql_error(), true
            );
        } else {
    ...
    public function getRawTaxRates()
    {
        ...
        if (!$result) {
            throw new ShopgateLibraryException(
                ShopgateLibraryException::PLUGIN_DATABASE_ERROR,
                "no tax rates found: " . mysql_error(), true
            );
        }
        ...
        foreach ($oscTaxRates as $oscTaxRate) {
            ...
            $taxRates[] = array(
                ...
                'state' => (!empty($oscTaxRate['countries_iso_code_2']) && !empty($oscTaxRate['zone_code']))
                    ? ShopgateMapper::getShopgateStateCode($oscTaxRate['countries_iso_code_2'], $oscTaxRate['zone_code'])
                    : '',
                'zip_code_type' => 'all',
            );
