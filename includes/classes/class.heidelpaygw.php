...
class heidelpayGW{
	...
	function heidelpayGW(){
		...
		if($this->hasRow){
			$this->modulConfButton = '<div class="add-margin-top-20"><a class="btn" href="' . GM_HTTP_SERVER . DIR_WS_ADMIN . 'admin.php?do=HeidelpayModuleCenterModule">' . HGW_TXT_CONFIG . '</a></div>';
		}
	...
	public function saveRes($paramsRaw){		
		...
		if(!empty($params['IDENTIFICATION.UNIQUEID'])){		
			...
			$serial			= addslashes(json_encode($params));
			$payType		= substr($params['PAYMENT.CODE'], 0, 2);
			$transType	= substr($params['PAYMENT.CODE'], 3, 2);
			
			$sql = 'SELECT `id` FROM `heidelpayGW_transactions` WHERE `IDENTIFICATION_UNIQUEID`= "'.addslashes($params['IDENTIFICATION.UNIQUEID']).'" ';
			...
			if(xtc_db_num_rows($query) > 0){
				...
				if($params['TRANSACTION.SOURCE'] == 'PUSH') {
					$sql = "UPDATE `heidelpayGW_transactions`
					SET PROCESSING_RESULT='".$params['PROCESSING.RESULT']."', PROCESSING_RETURN_CODE='".$params['PROCESSING.RETURN.CODE']."', PROCESSING_STATUS_CODE='".$params['PROCESSING.STATUS.CODE']."', TRANSACTION_SOURCE='".$params['TRANSACTION.SOURCE']."', IDENTIFICATION_REFERENCEID='".$params['IDENTIFICATION.REFERENCEID']."', jsonresponse='".$serial."' WHERE `id`= '".$res['id']."'";
				}else{
					$sql = "UPDATE `heidelpayGW_transactions`
					SET meth='".$payType."', type='". $transType."', IDENTIFICATION_UNIQUEID='".$params['IDENTIFICATION.UNIQUEID']."', IDENTIFICATION_SHORTID='".$params['IDENTIFICATION.SHORTID']."', IDENTIFICATION_TRANSACTIONID='".$params['IDENTIFICATION.TRANSACTIONID']."', IDENTIFICATION_REFERENCEID='".$params['IDENTIFICATION.REFERENCEID']."', PROCESSING_RESULT='".$params['PROCESSING.RESULT']."', PROCESSING_RETURN_CODE='".$params['PROCESSING.RETURN.CODE']."', PROCESSING_STATUS_CODE='".$params['PROCESSING.STATUS.CODE']."', TRANSACTION_SOURCE='".$params['TRANSACTION.SOURCE']."', TRANSACTION_CHANNEL='".$params['TRANSACTION.CHANNEL']."', jsonresponse='".$serial."', created=NOW()
					WHERE `id`= '".$res['id']."'";
				}
			}else{
				$sql = "
				INSERT INTO `heidelpayGW_transactions` (meth, type, IDENTIFICATION_UNIQUEID, IDENTIFICATION_SHORTID, IDENTIFICATION_TRANSACTIONID, IDENTIFICATION_REFERENCEID, PROCESSING_RESULT, PROCESSING_RETURN_CODE, PROCESSING_STATUS_CODE, TRANSACTION_SOURCE, TRANSACTION_CHANNEL, jsonresponse, created)				
				VALUES ('".$payType."', '".$transType."', '".$params['IDENTIFICATION.UNIQUEID']."', '".$params['IDENTIFICATION.SHORTID']."', '".$params['IDENTIFICATION.TRANSACTIONID']."', '".$params['IDENTIFICATION.REFERENCEID']."', '".$params['PROCESSING.RESULT']."', '".$params['PROCESSING.RETURN.CODE']."', '".$params['PROCESSING.STATUS.CODE']."', '".$params['TRANSACTION.SOURCE']."', '".$params['TRANSACTION.CHANNEL']."', '".$serial."', NOW())";
			}
			xtc_db_query($sql);
		}		
	}
	...
	public function includeCheckoutJs($pm = NULL){
		...
		if((!isset($js_setter)) || ($js_setter == $pm)){
			$js_setter = $pm;
			$checkoutJs = '
				<link type="text/css" rel="stylesheet" href="'.GM_HTTP_SERVER . DIR_WS_CATALOG . 'templates/EyeCandy/css/heidelpay.css" />';
/*				
...
*/
				...
				<script src="'.GM_HTTP_SERVER . DIR_WS_CATALOG . 'templates/EyeCandy/javascript/checkout/HeidelpayValPayment.js" type="text/javascript"></script>
				<div class="hp_error">
