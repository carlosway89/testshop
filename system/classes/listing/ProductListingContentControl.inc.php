<?php
/* --------------------------------------------------------------
   ProductListingContentControl.inc.php 2015-07-20 gm
   Gambio GmbH
   http://www.gambio.de
...
class ProductListingContentControl extends DataProcessing
{
		...
		if(empty($t_uninitialized_array))
		{
			...
			if($coo_listing_split->number_of_rows > 0)
			{
				...
				while($t_product_array = xtc_db_fetch_array($t_result))
				{
					...
					if($t_product_has_properties && $coo_product->data['use_properties_combis_shipping_time'] == '1')
					{
						$t_products_array[sizeof($t_products_array) - 1] = array_merge($t_products_array[sizeof($t_products_array) - 1],
																								   array('PRODUCTS_SHIPPING_NAME' => ''));
					}
				...
				$coo_product_listing_view->set_('get_params_hidden_data_array', $this->build_hidden_get_params_array());
		...
		if(empty($t_uninitialized_array))
		{
			...
			if(PRODUCT_LIST_FILTER > 0)
			{
				...
				if(xtc_db_num_rows($t_filterlist_query) > 1)
				{
					...
					$t_manufacturer_dropdown_html .= xtc_draw_hidden_field(xtc_session_name(), xtc_session_id());
	...
	public function determine_category_depth()
	{
		...
		if(empty($t_uninitialized_array))
		{
			...
			if((strpos(strtolower(gm_get_env_info("PHP_SELF")), FILENAME_DEFAULT) !== false &&
				$this->coo_filter_manager->is_active()) || isset($this->c_path) && xtc_not_null($this->c_path)
			)
			{
	...
	public function init_feature_filter()
	{
		...
		if(empty($t_uninitialized_array))
		{
			...
			)
			{
				...
				{
					$c_filter_categories_id = $this->feature_categories_id;
				}
	...
	public function determine_view_mode($p_view_mode_tiled)
	{
		$t_view_mode = '';
		...
		if(gm_get_conf('MAIN_VIEW_MODE_TILED') == 'true' || $p_view_mode_tiled == 1)
		{
			$t_view_mode = 'tiled';
		}
