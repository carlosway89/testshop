<?php
/* --------------------------------------------------------------
   CartActionsProcess.inc.php 2015-09-17
   Gambio GmbH
   http://www.gambio.de
...
class CartActionsProcess extends DataProcessing
{
	...
	protected function set_validation_rules()
	{
		...
		$this->validation_rules_array['coo_order'] = 					array('type' 		=> 'object',
																			  'object_type' => 'order');
		$this->validation_rules_array['coo_econda'] = 					array('type' 		=> 'object',
																			  'object_type' => 'econda');
	}

	public function proceed($p_action)
	{
	...
	{
		if($this->coo_econda != null)
		{
			$this->coo_econda->_emptyCart();
		}
		...
		for($i = 0, $n = sizeof($this->v_data_array['POST']['products_id']); $i < $n; $i++)
		{
			...
			else //CART ############
			{
				...
				if (in_array($this->v_data_array['POST']['products_id'][$i], $t_cart_delete_array))
				{
					...
					if($this->coo_econda != null)
					{
						$this->coo_econda->_delArticle($this->v_data_array['POST']['products_id'][$i], $t_gm_cart_products_qty, gm_convert_qty($this->v_data_array['POST']['old_qty'][$i]));
					}
					// BOF GM_MOD GX-Customizer
				...
				else
				{
					...
					if($this->coo_econda != null)
					{
						$old_quantity = $this->coo_cart->get_quantity(xtc_get_uprid($this->v_data_array['POST']['products_id'][$i], $this->v_data_array['POST']['id'][$i]));
						$this->coo_econda->_updateProduct($this->v_data_array['POST']['products_id'][$i], $t_gm_cart_products_qty, $old_quantity);
					}
	...
	protected function add_product(&$p_goto, &$p_parameters)
	{
		...
		if(isset($this->v_data_array['POST']['products_id']) && is_numeric($this->v_data_array['POST']['products_id']))
		{
			...
			{
				$this->coo_econda->_emptyCart();
				$old_quantity = $this->coo_cart->get_quantity(xtc_get_uprid($this->v_data_array['POST']['products_id'], $this->v_data_array['POST']['id'], (int)$t_products_properties_combis_id));
				$this->coo_econda->_addProduct($this->v_data_array['POST']['products_id'], $t_gm_cart_products_qty, $old_quantity);
			}
	...
	protected function add_a_quickie(&$p_goto, &$p_parameters)
	{
		...
		else
		{
			...
			if($this->v_data_array['POST']['quickie'] != '')
			{
				...
				// EOF GM_MOD
				$t_redirect_link = xtc_href_link($p_goto, xtc_get_all_get_params(array('action')) . '&open_cart_dropdown=1', 'NONSSL');
				$this->set_redirect_url($t_redirect_link);
				return;
			}
			else
			...
			*/
				...
				$this->coo_gprint_product_manager->get_surfaces_groups_id($this->v_data_array['GET']['BUYproducts_id']) !== false)
			{
				if((empty($this->v_data_array['POST']['products_id']) == false && (isset($this->v_data_array['POST']['id']) || $this->v_data_array['POST']['properties_values_ids'])) &&
					$this->coo_gprint_product_manager->get_surfaces_groups_id($this->v_data_array['GET']['BUYproducts_id']) === false)
				{
					if(is_object($this->coo_econda))
					{
						$this->coo_econda->_emptyCart();
						$t_econda_uprid = xtc_get_uprid($this->v_data_array['products_id'], $this->v_data_array['id'], (int)$t_products_properties_combis_id);
						$old_quantity = $this->coo_cart->get_quantity($t_econda_uprid);
						$this->coo_econda->_addProduct($t_econda_uprid, $t_gm_cart_products_qty, $old_quantity);
					}
			...
			else
			{
				if (isset($this->coo_cart))
				{
					...
					{
						$this->coo_econda->_emptyCart();
						$old_quantity = $this->coo_cart->get_quantity((int)$this->v_data_array['GET']['BUYproducts_id']);
						$this->coo_econda->_addProduct((int)$this->v_data_array['GET']['BUYproducts_id'], $t_gm_cart_products_qty, $old_quantity);
					}
					// BOF GM_MOD
