...
class ShopgateItemXmlModel extends ShopgateItemModel
{
    ...
    public function setPrice()
    {
        ...
        if ($this->getIsChild()) {
            ...
            if (!empty($this->cache['currentChild']['products_vpe_value'])
                && !empty($this->cache['currentChild']['products_vpe_name'])
            ) {
                $combinationPrice = 0;
                if ($this->cache['currentChild']['combi_price_type'] == 'calc') {
                    $combinationPrice = $price * (1 / $this->cache['currentChild']['products_vpe_value']);
                } elseif ($this->cache['currentChild']['combi_price_type'] == 'fix') {
                    $combinationPrice = $salesPrice * (1 / $this->cache['currentChild']['products_vpe_value']);
                }
            ...
            $vpe = "";
            if (!empty($this->item['products_vpe_name']) && !empty($this->item['products_vpe_value'])) {
                $vpePrice = $this->addTaxes($price, $taxRate);
                $vpePrice = $vpePrice * (1 / $this->item['products_vpe_value']);
    ...
    public function setAttributes()
    {
        ...
        if ($this->checkAttributeIsProperty()) {
            ...
            $groupItems = $this->cache['currentChild']['group_data'];
            
            if (empty($parentAttGroups)) {
                foreach ($groupItems AS $key => $groupItem) {
                    $attributeGroupItem = new Shopgate_Model_Catalog_AttributeGroup();
                    $attributeGroupItem->setUid(bin2hex($groupItem));
                    $attributeGroupItem->setLabel($groupItem);
            ...
            for ($i = 1; $i <= count($this->cache['currentChild']['group_data']); $i++) {
                ...
                $inputObject->setGroupUid(
                    !empty($child['group_data']["attribute_{$i}"])
                        ? bin2hex($child['group_data']["attribute_{$i}"])
                        : ""
                );
