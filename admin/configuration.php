			...
			<table border="0" width="100%" cellspacing="2" cellpadding="0">
				<tr>
					...
					<td class="boxCenter" width="100%" valign="top">
						<div class="pageHeading" style="background-image:url(images/gm_icons/meinshop.png)">
							<?php
										...
										$("#shop_key_data").html($("#shop_key_data").html().replace(/shop_key=.*?\nlanguage/g, "shop_key=" + $(this).val() + "\nlanguage"));
													...
													switch($configuration['configuration_key'])
													{
														...
														case 'PAYPAL_SHOP_LOGO':
															$t_cfg_desc .= HTTP_SERVER . DIR_WS_CATALOG . DIR_WS_IMAGES . 'logos/' . gm_get_conf('GM_LOGO_SHOP');
															break;
													...
													if($t_show_option)
													{
														...
														if ($configuration['configuration_key'] != 'SESSION_FORCE_COOKIE_USE')
														{
															$t_configuration_html = '<td class="dataTableContent_gm' . (isset($_SESSION['configuration_validation_error_values'][$configuration['configuration_key']]) ? ' error' : '') . '">'.$value_field.'<span class="tooltip-icon" data-gx-widget="tooltip_icon" data-tooltip_icon-type="info">' . $t_cfg_desc . $gm_vat_live_check . '</span></td>';

															if($t_shop_key_textarea !== '')
															{
																$t_configuration_html = '<td class="dataTableContent_gm' . (isset($_SESSION['configuration_validation_error_values'][$configuration['configuration_key']]) ? ' error' : '') . '">'.$value_field. '<div style="clear: left; padding: 10px 0">' . $t_cfg_desc. '</div>' . $t_shop_key_textarea . '</td>';
															}
