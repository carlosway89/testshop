<?php
/* --------------------------------------------------------------
	PaymentInstructionInvoiceExtender.inc.php 2015-11-12
	Gambio GmbH
	http://www.gambio.de
...
class PaymentInstructionInvoiceExtender extends PaymentInstructionInvoiceExtender_parent
{
	...
	protected function _getPaymentInstruction($orders_id)
	{
		...
		if($paymentInstruction !== null && $this->_getPaymentMethod($orders_id) === 'paypal3')
		{
			...
			$paymentInstruction['additional_note'] = STORE_OWNER.' '.$paypalText->get_text('payment_instruction_additional_note');
		}
	...
	protected function _makePaymentInstructionText($paymentInstruction)
	{
		$textLines = array(
			...
			$this->paymentInstructionText->get_text('due_date'). ': '. $paymentInstruction['due_date'],
		);
