/* --------------------------------------------------------------
 orders_modal_layer.js 2015-10-16 gm
 Gambio GmbH
 http://www.gambio.de
...
gx.compatibility.module(
	...
	function (data) {
		...
		var _openDeleteDialog = function (event) {
			...
			$form.dialog({
				             'title': jse.core.lang.translate('TEXT_INFO_HEADING_DELETE_ORDER', 'orders').replace('%s', $this.data(
					             'order_id')),
				             'modal': true,
		...
		var _openTrackingCodeDeleteDialog = function (event) {
			...
			var data_set = module._data($(this));
			$form.dialog({
				             'title': jse.core.lang.translate('TXT_PARCEL_TRACKING_DELETE_BUTTON', 'parcel_services').replace(
				             ...
				             'buttons': [
					             ...
					             },
					             {
						             ...
						             'click': function () {
							             $(this).dialog('close');
							             jse.libs.xhr.post({
								                               'url': 'request_port.php?module=ParcelServices&action=delete_tracking_code',
								                               'data': {
		...
		var _openMultiDeleteDialog = function (event) {
			...
			$form.dialog({
				             'title': jse.core.lang.translate('TEXT_INFO_HEADING_MULTI_DELETE_ORDER', 'orders').replace('%s',
				                                                                                                        $this.data('order_id')),
				             'modal': true,
				             'dialogClass': 'gx-container',
				             'buttons': _getModalButtons($form),
		...
		var _openMultiCancelDialog = function (event) {
			...
			$form.dialog({
				             'title': jse.core.lang.translate('TEXT_INFO_HEADING_MULTI_CANCEL_ORDER', 'orders').replace('%s',
				                                                                                                        $this.data('order_id')),
				             'modal': true,
				             'dialogClass': 'gx-container',
				             'buttons': _getModalButtons($form),
		...
		var _openTrackingCodeDialog = function (event) {
			...
			event.preventDefault();
			$form.dialog({
				             'title': jse.core.lang.translate('TXT_PARCEL_TRACKING_HEADING', 'parcel_services').replace('%s',
				                                                                                                        $this.data('order_id')),
				             'modal': true,
				             'dialogClass': 'gx-container',
				             'buttons': _getModalButtons($form),
