<?php
/* --------------------------------------------------------------
   orders_edit_properties.php 2014-08-15 gm
   Gambio GmbH
   http://www.gambio.de
...
{
	$t_combis_data_array = $coo_properties_control->get_selected_combi($c_products_id, $order->info['languages_id'], $_POST['properties_values_ids']);
	...
	if(isset($_POST['save_properties']))
	{
		...
		{
			$t_sql = 'INSERT INTO orders_products_properties
						SET
							orders_products_id = "' . $c_orders_products_id . '",
							products_properties_combis_id = "' . (int)$t_combis_data_array['products_properties_combis_id'] . '",
							properties_name = "' . xtc_db_input($t_combi_value_array['properties_name']) . '",
							values_name = "' . xtc_db_input($t_combi_value_array['values_name']) . '",
							properties_price_type = "' . xtc_db_input($t_combis_data_array['combi_price_type']) . '",
							properties_price = "' . (double)$t_combi_value_array['value_price'] . '"';
			xtc_db_query($t_sql);
		}
	}
				...
				$.ajax({
					...
					success: function(p_response) 
					{
						if(p_response.status != "no_combi_selected" && p_response.status != "combi_not_exists")
						{
							...
							{
								$("dd.shipping_time img").attr("src", "admin/images/icons/" + p_response.shipping_status.shipping_status_image);
